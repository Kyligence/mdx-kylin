...
public class BatchDatasetService {
    ...
    public Response<DatasetIdVO> doInsert(DatasetDTO datasetDTO, String basicAuth, boolean insertOnUpdate,
        ...
        List<KylinGenericModel> modelsByKe = SemanticAdapter.INSTANCE.getNocacheGenericModels(datasetDTO.getProject());
        List<String> modelNamesByKe = modelsByKe.stream().map(model -> model.getModelName()).collect(Collectors.toList());
        List<CommonDimModelDTO> commonDimRelations = datasetDTO.getModelRelations();
        commonDimRelations.forEach(modelRelation -> {
            if(!modelRelation.getModelLeft().isEmpty() && !modelNamesByKe.contains(modelRelation.getModelLeft())){
                modelValid.set(false);
                return;
            }
            if(!modelRelation.getModelRight().isEmpty() && !modelNamesByKe.contains(modelRelation.getModelRight())){
                modelValid.set(false);
                return;
            }
        });
