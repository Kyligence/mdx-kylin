...
import mondrian.xmla.constants.XmlaHandlerConstants;
...
public abstract class Rowset implements XmlaConstants {
    ...
    public final RowsetDefinition rowsetDefinition;
    ...
    public static String RAW_VALUE_IDENTIFIER = "<Raw>: ";
...
//                    + "' does not support property '" + propertyName + "'");
    ...
    protected void setProperty(PropertyDefinition propertyDef, String value) {
        switch (propertyDef) {
            ...
            case LocaleIdentifier:
                if (value != null) {
                    try {
                        ...
                        if (locale != null) {
                            extraProperties.put(
                                    XmlaHandlerConstants.JDBC_LOCALE, locale.toString());
                            return;
                        }
            ...
            case DbpropMsmdSubqueries:
            case VisualMode:

            case MdxMissingMemberMode:
            case DbpropMsmdMDXCompatibility:
            case SafetyOptions:
            case DbpropMsmdOptimizeResponse:
                extraProperties.put(propertyDef.name(), value);
                break;
     ...
     */
    public final void unparse(XmlaResponse response, SaxWriter writer)
            throws XmlaException {
        unparseColumns(response, writer, rowsetDefinition.columnDefinitions);
    }

    public final void unparseColumns(XmlaResponse response, SaxWriter writer, RowsetDefinition.Column[] columns)
            throws XmlaException {
        ...
        for (Row row : rows) {
            emit(row, response, writer, columns);
        }
        writer.endSequence();
    }
    ...
    public final void populate(
        ...
        try {
            ...
            request1 = handler.connectionFactory.startRequest(request, connection);
     ...
     * @param columns  Columns limitation
     */
    private void emit(Row row, XmlaResponse response, SaxWriter writer, RowsetDefinition.Column[] columns)
            throws XmlaException {
        ...
        for (RowsetDefinition.Column column
            ...
            if (value == null) {
                ...
                for (Row row1 : rows) {
                    rowset.emit(row1, response, writer, columns);
                }
                writer.endSequence();
            } else if (value instanceof PlainText) {
                PlainText text = (PlainText) value;
                writer.pureCharacters(text.text);
            } else {
    ...
    public String[] emitValue(Row row, List<Integer> columnIndices, RowsetDefinition.Column[] columns)
            throws XmlaException, SQLException {
        String[] rowValues = new String[columnIndices.size()];

        for (int i = 0; i < columnIndices.size(); i++) {
            StringBuilder sb = new StringBuilder();
            SaxWriter writer = new DefaultSaxWriter(sb);

            RowsetDefinition.Column column = columns[columnIndices.get(i)];
            Object value = row.get(column.name);
            ...
            if (value == null) {
                ...
                sb.append(RAW_VALUE_IDENTIFIER);
                XmlElement[] elements = (XmlElement[]) value;
                for (XmlElement element : elements) {
                    emitXmlElement(writer, element);
                }
                ...
                Rowset rowset = (Rowset) value;
                final List<Row> rows = new ArrayList<>();
                rowset.populate(null, null, rows);
                writer.startSequence(column.name, "row");
                for (Row row1 : rows) {
                    rowset.emit(row1, null, writer, columns);
                }
                writer.endSequence();
            } else if (value instanceof PlainText) {
                sb.append(RAW_VALUE_IDENTIFIER);
                ...
                writer.textCharacters(value.toString());
            }
            rowValues[i] = sb.toString();
        }
    ...
    <E, V> Util.Predicate1<E> makeCondition(
        ...
        } else if (restriction instanceof DMVParser.DMVRestriction) {
            DMVParser.DMVRestriction<V> dmvRestriction = (DMVParser.DMVRestriction<V>) restriction;
            return new Util.Predicate1<E>() {
                public boolean test(E element) {
                    V value = getter.apply(element);
                    return dmvRestriction.check(value);
                }
            };
        } else {
    ...
    protected Util.Predicate1<Catalog> catNameCond() {
        ...
        final String attempCatalogName = properties.get(PropertyDefinition.Catalog.name());
        final String catalogName = attempCatalogName == null ? properties.get(PropertyDefinition.Cube.name()) : attempCatalogName;
        if (catalogName != null) {
    ...
    public RowsetDefinition getRowsetDefinition() {
        return rowsetDefinition;
    }
}
