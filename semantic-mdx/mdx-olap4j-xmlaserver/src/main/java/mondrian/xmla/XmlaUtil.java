...
public class XmlaUtil implements XmlaConstants {
    ...
    public static RowsetDefinition.Column[] findColumns(Collection<String> findingTargets, RowsetDefinition.Column[] columns) {
        List<RowsetDefinition.Column> foundColumns = new ArrayList<>(findingTargets.size());

        for (RowsetDefinition.Column column : columns) {
            if (findingTargets.contains(column.name)) {
                foundColumns.add(column);
            }
        }

        return foundColumns.toArray(new RowsetDefinition.Column[0]);
    }

    public static Pair<List<Integer>, List<String>> findColumnIndicesNames(Collection<String> findingTargets, RowsetDefinition.Column[] columns) {
        List<Integer> foundColumnIndices = new ArrayList<>(findingTargets.size());
        List<String> foundColumnNames = new ArrayList<>(findingTargets.size());

        for (int i = 0; i < columns.length; i++) {
            if (findingTargets.contains(columns[i].name)) {
                foundColumnIndices.add(i);
                foundColumnNames.add(columns[i].name);
            }
        }

        return new Pair<>(foundColumnIndices, foundColumnNames);
    }
    ...
    private static String encodeChar(char c) {
        ...
        return buf.append(str).append("_").toString().toUpperCase();
    }
...
*/
    ...
    public static MetadataRowset getMetadataRowset(
        ...
        final ConnectionFactory connectionFactory =
                new ConnectionFactory() {
                    ...
                    public XmlaExtra getExtra() {
                        try {
                            return connection.unwrap(XmlaExtra.class);
        ...
        final Rowset rowset =
                rowsetDefinition.getRowset(request,
                        XmlaHandlerFactory.createHandler(connectionFactory, "xmla", connection)
                );
