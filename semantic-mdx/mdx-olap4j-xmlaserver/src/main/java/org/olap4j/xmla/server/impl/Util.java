...
public class Util {
    ...
    /**
     * Extract the catalog name from a mdx statement for MSTR with one database instance for multi datasets
     * @param mdxStatement
     * @return The catalog name
     */
    public static String extractCatalogFromMdx(String mdxStatement) {
        String catalogRegex = "(?i).*from\\s\\[([^\\]]*)\\].*";
        Pattern catalogPattern = Pattern.compile(catalogRegex);
        Matcher catalogMatcher = catalogPattern.matcher(mdxStatement.replaceAll("\n", ""));
        if (catalogMatcher.matches()) {
            return catalogMatcher.group(1);
        }
        return null;
    }
