...
public class IfEndsWithFromRule extends MdxTransferRule {
    private static final Pattern endsWithFromPattern = Pattern.compile("(?i)from[\\s]*$");

    @Override
    public String apply(String mdx) {
        Matcher endsWithFromMatcher = endsWithFromPattern.matcher(mdx);
        if (endsWithFromMatcher.find()) {
            XmlaRequestContext context = XmlaRequestContext.getContext();
            String cubeName = context.getParameter("Catalog");
            mdx = endsWithFromMatcher.replaceFirst("from [" + cubeName + "]");
        }
        return mdx;
    }
}
