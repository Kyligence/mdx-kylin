...
SELECT
  NON EMPTY Hierarchize(Intersect(AddCalculatedMembers({DrilldownLevel({[KYLIN_ACCOUNT].[ACCOUNT_BUYER_LEVEL].[All]})}), [XL_Col_Dim_0])) DIMENSION PROPERTIES PARENT_UNIQUE_NAME ON COLUMNS,
...
FROM [learn_kylin]
WHERE ([Measures].[_COUNT_], [KYLIN_ACCOUNT].[ACCOUNT_COUNTRY].[Slicer_0])
CELL PROPERTIES [VALUE],[FORMAT_STRING],[LANGUAGE],[BACK_COLOR],[FORE_COLOR],[FONT_FLAGS]