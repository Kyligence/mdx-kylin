...
export CP="test-lib/mondrian.jar"
export CP="${CP}${PS}lib/olap4j.jar"
export CP="${CP}${PS}test-lib/mondrian-tests.jar"
...
mysql() {
    java -cp "${CP}${PS}test-lib/mysql-connector-java-5.1.17.jar" \
        mondrian.test.loader.MondrianFoodMartLoader \
        -verbose -aggregates -tables -data -indexes \
        -dataset=${dataset} \
        ...
        -afterFile="$afterFile" \
        -outputQuoted=${outputQuoted} \
        -outputJdbcURL="jdbc:mysql://10.1.2.21:3306/${datasetLower}?user=root&password="
}
