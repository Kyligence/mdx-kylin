...
public class JdbcDialectImpl implements Dialect {
    ...
    @Override
    public String caseWhenTestElse(String[] expressions) {
        assert expressions.length >= 2;
        StringBuilder sb = new StringBuilder("CASE");
        for (int i = 0; i < expressions.length / 2; i++) {
            sb.append(" WHEN ");
            sb.append(expressions[i * 2]);
            sb.append(" THEN ");
            sb.append(expressions[i * 2 + 1]);
        }
        if (expressions.length % 2 == 1) {
            sb.append(" ELSE ");
            sb.append(expressions[expressions.length - 1]);
        }
        sb.append(" END");
        return sb.toString();
    }
    ...
    @Override
    public String caseWhenMatchElse(String[] expressions) {
        assert expressions.length >= 3;
        StringBuilder sb = new StringBuilder("CASE ");
        sb.append(expressions[0]);
        for (int i = 1; i < (expressions.length + 1) / 2; i++) {
            sb.append(" WHEN ");
            sb.append(expressions[i * 2 - 1]);
            sb.append(" THEN ");
            sb.append(expressions[i * 2]);
