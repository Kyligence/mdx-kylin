...
public class SegmentHeader implements Serializable {
    ...
    private int computeHashCode() {
        ...
        hash = Util.hash(hash, customRollupRequirements);
        return hash;
    }
    ...
    public ByteString getUniqueID() {
        if (this.uniqueID == null) {
            ...
            if (customRollupRequirements != null) {
                for (String alias : customRollupRequirements.getNonQueryingM2MTableAliases()) {
                    hashSB.append(alias);
                }
                for (String sql : customRollupRequirements.getWeightColumnSqls()) {
                    hashSB.append(sql);
                }
            }
            this.uniqueID =
