...
public interface Dialect {
    ...
    /**
     * Generates a conditional statement in this dialect's syntax.
     * This method deals with more common conditions, where the count of expressions is flexible.
     *
     * <p>For example, {@code caseWhenTestElse({}"a = 1", "'A'", "a = 2", "'B'", "'NULL'"})} ->
     *     {@code "CASE WHEN a = 1 THEN 'A' WHEN a = 2 THEN 'B' ELSE 'NULL' END"}
     * @param expressions A flexible array of expressions.
     * @return Conditional expression.
     */
    String caseWhenTestElse(String[] expressions);

    /**
     * Generates a conditional statement in this dialect's syntax.
     * This method deals with more common conditions, where the count of expressions is flexible.
     *
     * <p>For example, {@code caseWhenMatchElse({"a", "1", "'A'", "2", "'B'", "'NULL'"})} ->
     *     {@code "CASE a WHEN 1 THEN 'A' WHEN 2 THEN 'B' ELSE 'NULL' END"}
     * @param expressions A flexible array of expressions.
     * @return Conditional expression.
     */
    String caseWhenMatchElse(String[] expressions);
    ...
    enum Datatype {
        ...
        private static boolean toBoolean(Object value) {
            ...
            if (value instanceof Boolean) {
                ...
                if (valueString.equalsIgnoreCase("TRUE") || "1".equals(valueString)) {
                    return true;
                } else if (valueString.equalsIgnoreCase("FALSE") || "0".equals(valueString)) {
                    return false;
                } else {
