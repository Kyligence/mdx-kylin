...
public class MondrianServerRegistry {
    ...
    public static ConcurrentHashMap<String, FileRepository>  repositoryMap = new  ConcurrentHashMap<>();
        ...
        CatalogLocator catalogLocator)
    {
        ...
        if (contentFinder == null) {
            ...
            if (contentFinder instanceof UrlRepositoryContentFinder) {
                String url = ((UrlRepositoryContentFinder) contentFinder).getUrl();
                int start = url.lastIndexOf("/");
                int last = url.lastIndexOf(".");
                String repositoryId = url.substring(start + 1, last);
                repositoryMap.put(repositoryId, (FileRepository) repository);
            }
        }
        return new MondrianServerImpl(this, repository, catalogLocator);
    }

    public void checkDataset(String repositoryId, String dataset) throws Exception {
        FileRepository repository = repositoryMap.get(repositoryId);
        if (repository == null) {
            return;
        }
        String datasetId = repositoryId.substring(0, repositoryId.lastIndexOf("_")) + "_" + dataset;
        repository.checkDataset(datasetId);
    }
