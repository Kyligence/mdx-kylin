...
import mondrian.rolap.CustomRollupRequirements;
...
public class SegmentCacheIndexImpl implements SegmentCacheIndex {
    ...
    private final Map<List, CopyOnWriteArrayList<SegmentHeader>> bitkeyMap =
        ...
        boolean loading)
    {
        ...
        headerMap.putIfAbsent(header, new HeaderInfo(loading ? new SlotFuture<>() : null));

        final List bitkeyKey = makeBitkeyKey(header);
        CopyOnWriteArrayList<SegmentHeader> headerList =
                bitkeyMap.computeIfAbsent(bitkeyKey, ignored -> new CopyOnWriteArrayList<>());
        headerList.addIfAbsent(header);

        final List factKey = makeFactKey(header);
        FactInfo factInfo = factMap.computeIfAbsent(factKey, ignored -> new FactInfo());
        factInfo.headerSet.add(header);
        ...
        FuzzyFactInfo fuzzyFactInfo = fuzzyFactMap.computeIfAbsent(fuzzyFactKey, ignored -> new FuzzyFactInfo());
        fuzzyFactInfo.headerSet.add(header);
    }
        ...
        SegmentHeader newHeader)
    {
        ...
        synchronized (factInfo.headerSet) {
            factInfo.headerSet.remove(oldHeader);
            factInfo.headerSet.add(newHeader);
        }

        final List oldFuzzyFactKey = makeFuzzyFactKey(oldHeader);
        final FuzzyFactInfo fuzzyFactInfo = fuzzyFactMap.get(oldFuzzyFactKey);
        synchronized (fuzzyFactInfo.headerSet) {
            fuzzyFactInfo.headerSet.remove(oldHeader);
            fuzzyFactInfo.headerSet.add(newHeader);
        }
    }
...
//            headerInfo.slot.put(body);
            headerInfo.slot.put(SegmentBody.EMPTY);
        }
    ...
    public void remove(SegmentHeader header) {
        ...
        if (factInfo != null && factInfo.isEmptyAfterRemove(header)) {
            factMap.remove(factKey);
        }

        final List fuzzyFactKey = makeFuzzyFactKey(header);
        final FuzzyFactInfo fuzzyFactInfo = fuzzyFactMap.get(fuzzyFactKey);
        if (fuzzyFactInfo != null && fuzzyFactInfo.isEmptyAfterRemove(header)) {
        ...
        if (Utils.isCollectionEmpty(headerList)) {
        ...
        SegmentColumn[] region)
    {
        ...
        for (SegmentHeader header : factInfo.headerSet) {
    ...
    private List makeBitkeyKey(SegmentHeader header) {
        return makeBitkeyKey(
                ...
                header.measureName,
                header.compoundPredicates,
                header.getCustomRollupRequirements());
    }
    ...
    private static class FactInfo {
        ...
        private final Set<SegmentHeader> headerSet =
        ...
        private synchronized boolean isEmptyAfterRemove(SegmentHeader header) {
            headerSet.remove(header);
            bitkeyPoset.remove(header.getConstrainedColumnsBitKey());
