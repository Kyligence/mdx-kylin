...
public class RolapEvaluator implements Evaluator {
    ...
    // #125 Fix sort or limit not correct if query has filter condition
    ...
    public RolapMember[] getNonAllMembers() {
        ...
        //Since root is shared among evaluators generated by evaluator.push()
        // and is not restored by evaluator.restore(), the result may contain All member
        //Thus, the result needs to be filtered before returned
        if (nonAllMembers.length > 0) {
            List<RolapMember> tempList = new ArrayList<RolapMember>();
            for (RolapMember nonAllMember : nonAllMembers) {
                if (!nonAllMember.isAll() || nonAllMember.getHierarchy().isCustomRollup()) {
                    tempList.add(nonAllMember);
                }
            }
            nonAllMembers = tempList.toArray(new RolapMember[0]);
        }
    ...
    public final Member setContext(Member member) {
        return setContext(true, member);
    }

    public final Member setContext(boolean removeAggregationList, Member member) {
        return setContext(removeAggregationList, member, false);
    }

    public final Member setContext(boolean removeAggregationList, Member member, boolean forceAll) {
        ...
        if (removeAggregationList) {
            getAggregationList(member, true);
        }
        ...
        if (previous.isAll() && (forceAll || !m.isAll()) && isNewPosition(ordinal)) {
    ...
    @Override
    public final List<List<Member>> getAggregationList(Member member, boolean remove) {
        if (member.isMeasure() || aggregationLists == null) {
            return null;
        }

        RolapMember m = (RolapMember)member;
        Hierarchy toSetHierarchy = m.getHierarchy();

        Iterator<List<List<Member>>> aggregationListIterator = aggregationLists.listIterator();
        List<List<Member>> found = null;
        while (aggregationListIterator.hasNext()) {
            List<List<Member>> aggregationList = aggregationListIterator.next();
            Hierarchy alreadyHierarchy = aggregationList.get(0).get(0).getHierarchy();

            if (toSetHierarchy.equals(alreadyHierarchy) && !(m instanceof VisualTotalsFunDef.VisualTotalMember)) {
                if (remove) {
                    aggregationListIterator.remove();
                }
                found = aggregationList;
            }
        }
    ...
    public final void setContextIncludingCustomRollupAll(List<Member> memberList) {
        for (int i = 0, n = memberList.size(); i < n; i++) {
            Member member = memberList.get(i);
            assert member != null : "null member in " + memberList;
            Hierarchy hierarchy = member.getHierarchy();
            if (hierarchy instanceof RolapCubeHierarchy
                    && ((RolapCubeHierarchy)hierarchy).isCustomRollup()
                    && member.isAll()) {
                setContext(true, member, true);
            } else {
                setContext(member);
            }
        }
    ...
    public final Object evaluateCurrent() {
        ...
        Object nonEmptyStatus = checkNonEmptyMeasures(maxSolveMember);
        if (nonEmptyStatus == RolapUtil.valueNotReadyException) {
            return RolapUtil.valueNotReadyException;
        }
        if (nonEmptyStatus == Boolean.TRUE) {
            return null;
        }
    ...
    private Object checkNonEmptyMeasures(RolapCalculation maxSolveMember) {
        if (!(maxSolveMember instanceof RolapHierarchy.RolapCalculatedMeasure)) {
            return Boolean.FALSE;
        }

        RolapHierarchy.RolapCalculatedMeasure calculatedMeasure =
                ((RolapHierarchy.RolapCalculatedMeasure)maxSolveMember);
        if (Utils.isCollectionEmpty(calculatedMeasure.getNonEmptyBehaviourMeasures())) {
            return Boolean.FALSE;
        }

        for (RolapMember measureMember : calculatedMeasure.getNonEmptyBehaviourMeasures()) {
            final int savepoint = savepoint();
            try {
                setContext(measureMember);
                final Object o = evaluateCurrent();
    ...
    public final String getFormatString() {
        ...
        if (formatExp == null) {
            return RolapUtil.DEFAULT_FORMAT_STRING_TAG;
        }
    ...
    private RolapCalculation getAbsoluteMaxSolveOrder() {
        ...
        for (int i = 1; i < calculationCount; i++) {
            ...
            if (expandsBefore(member, maxSolveMember)
                    ...
                    || ((VisualTotalsFunDef.VisualTotalMember) member).getMember().isCalculated())) {
