...
public class SegmentLoader {
        ...
        implements Callable<Map<Segment, SegmentWithData>>
    {
        ...
        public Map<Segment, SegmentWithData> call() throws Exception {
            ...
            try {
                ...
                RolapUtil.removeLog4jQueryIdRecord();
                Locus.pop(locus);
            }
        }
    }
        ...
        List<StarPredicate> compoundPredicateList)
    {
        ...
        final Util.Function1<Statement, Void> callbackWithCaching =
            new Util.Function1<Statement, Void>() {
                @Override
        ...
        final Util.Function1<Statement, Void> callbackNoCaching =
                new Util.Function1<Statement, Void>() {
                    @Override
                    public Void apply(final Statement stmt) {
                 ...
                 axisIndex++, columnIndex++)
            {
                ...
                case DATE:
                    ...
                    if (dateValue == null && rawRows.wasNull()) {
                        ...
                        axisValueSets[axisIndex].add(dateValue);
                        processedRows.setDate(columnIndex, dateValue);
                    }
                    break;
                case BOOLEAN:
                    final Boolean booleanValue =
                            rawRows.getBoolean(columnIndex + 1);
                    if (rawRows.wasNull()) {
                        if (!groupingSetsList.useGroupingSets()
                                || !isAggregateNull(
                                rawRows,
                        ...
                        axisValueSets[axisIndex].add(booleanValue);
                        processedRows.setBoolean(columnIndex, booleanValue);
                    }
                    break;
                case TIMESTAMP:
                    final Timestamp timestampValue =
                            rawRows.getTimestamp(columnIndex + 1);
                    if (timestampValue == null && rawRows.wasNull()) {
                        ...
                        axisValueSets[axisIndex].add(timestampValue);
                        processedRows.setTimstamp(columnIndex, timestampValue);
                    }
                    break;
                default:
                    throw Util.unexpected(type);
...
BitKey getRollupBitKey(int arity, ResultSet rowList, int k)
     ...
     */
    public static boolean useSparse(
    ...
    protected static class RowList {
        ...
        public void setBoolean(int column, Boolean value) {
            columns[column].setBoolean(currentRow, value);
        }

        public void setTimstamp(int column, Timestamp value) {
        ...
        static abstract class Column {
                ...
                int capacity)
            {
                ...
                case BOOLEAN:
                    return new BooleanColumn(ordinal, type, capacity);
                case TIMESTAMP:
        ...
        static class DateColumn extends NativeColumn {
            private Date[] dates;
            ...
            public void populateFrom(int row, ResultSet resultSet)
                    throws SQLException
            {
                Date d = dates[row] = resultSet.getDate(ordinal + 1);
                if (d == null) {
                    getNullIndicators().set(row, resultSet.wasNull());
                }
            }
        ...
        static class BooleanColumn extends NativeColumn {
            private Boolean[] booleans;

            BooleanColumn(int ordinal, SqlStatement.Type type, int size) {
                super(ordinal, type);
            ...
            @Override
            public void resize(int newSize) {
                booleans = Util.copyOf(booleans, newSize);
            }
            ...
            public void populateFrom(int row, ResultSet resultSet)
                    throws SQLException
            {
                Boolean b = booleans[row] = resultSet.getBoolean(ordinal + 1);
                if (b == null) {
                    getNullIndicators().set(row, resultSet.wasNull());
                }
            }
            ...
            @Override
            public boolean isNull(int row) {
                return booleans[row] == null
                        && nullIndicators != null
                        && nullIndicators.get(row);
            }

            @Override
            public Boolean getObject(int row) {
                return isNull(row) ? null : booleans[row];
            }
        }

        static class TimestampColumn extends NativeColumn {
            ...
            public void populateFrom(int row, ResultSet resultSet)
                    throws SQLException
            {
                Timestamp t = timestamps[row] = resultSet.getTimestamp(ordinal + 1);
                if (t == null) {
                    getNullIndicators().set(row, resultSet.wasNull());
                }
            }
            ...
            @Override
            public Timestamp getObject(int row) {
