...
public class ListColumnPredicate extends AbstractColumnPredicate {
     ...
     * hash code.
     */
    private Set<StarColumnPredicateWrapper> childrenSet;
    ...
    public Set<StarColumnPredicateWrapper> getPredicatesSet() {
        if (childrenSet == null) {
    ...
    private synchronized void initializeChildrenSet() {
        if (childrenSet == null) {
            Set<StarColumnPredicateWrapper> newChildrenSet = new HashSet<>(children.size());
            for (StarColumnPredicate starColumnPredicate : children) {
                newChildrenSet.add(new StarColumnPredicateWrapper(starColumnPredicate));
            }
    ...
    public boolean equalConstraint(StarPredicate that) {
        if (!(that instanceof ListColumnPredicate)) {
            return false;
        }
        ...
        if (getPredicates().size() != thatPred.getPredicates().size()) {
            ...
            return SetUtils.isEqualSet(getPredicatesSet(), thatPred.getPredicatesSet());
        }
    }
    ...
    private static class StarColumnPredicateWrapper {
        private final StarColumnPredicate starColumnPredicate;

        public StarColumnPredicateWrapper(StarColumnPredicate starColumnPredicate) {
            assert starColumnPredicate != null;
            this.starColumnPredicate = starColumnPredicate;
        }

        public StarColumnPredicate getStarColumnPredicate() {
            return starColumnPredicate;
        }

        @Override
        public int hashCode() {
            return starColumnPredicate.hashCode();
        }

        @Override
        public boolean equals(Object o) {
            if (!(o instanceof StarColumnPredicateWrapper)) {
                return false;
            }
            return starColumnPredicate.equalConstraint(((StarColumnPredicateWrapper)o).getStarColumnPredicate());
        }
    }
}
