...
public abstract class AbstractQuerySpec implements QuerySpec {
        ...
        final SqlQueryBuilder queryBuilder)
    {
        ...
        String exprInner = generateMeasureInnerExpression(measure, queryBuilder);

        if (measure.getExpression() == null) {
            // add from clause for count(*) measure, 默认使用 fact table
            queryBuilder.addRelation(queryBuilder.table(measure.getTable()), SqlQueryBuilder.NullJoiner.INSTANCE);
        } else {
            SqlQueryBuilder.Column column = queryBuilder.column(measure.getExpression(), measure.getTable());
        ...
        if (queryBuilder.sqlQuery instanceof CustomizeSqlQuery) {
            ...
            customizeSqlQuery.addMeasure(measure);
        }
    }

    protected String generateMeasureInnerExpression(RolapStar.Measure measure, SqlQueryBuilder queryBuilder) {
        return measure.getExpression() == null ? "*" : measure.getExpression().toSql();
    }
    ...
    public Pair<String, List<SqlStatement.Type>> generateSqlQuery(String desc) {
        ...
        if (queryBuilder.sqlQuery != null) {
            queryBuilder.sqlQuery.setCellCalc(true);
        }

        if (queryBuilder.sqlQuery instanceof CustomizeSqlQuery) {
            Pair<RolapStar.Table, Map<String, RolapSchema.PhysPath>> factAndPath = star.getFactAndPath();
            String m2mPk = star.getManyToManyPk();
            String bridgeTable = star.getBridgeTable();
            return queryBuilder.toSqlAndTypes(factAndPath, m2mPk, bridgeTable);
        }
    ...
    protected void extraPredicates(SqlQueryBuilder queryBuilder) {
        for (StarPredicate predicate : getPredicateList()) {
            for (PredicateColumn pair : predicate.getColumnList()) {
                SqlQueryBuilder.Column column = queryBuilder.column(pair.physColumn, pair.router);
