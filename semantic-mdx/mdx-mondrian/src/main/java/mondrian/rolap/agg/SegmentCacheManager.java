...
public class SegmentCacheManager {
    ...
    public final ExecutorService sqlExecutor =
        Util.getExecutorService(
                ...
                MondrianProperties.instance().SegmentCacheManagerCoreSqlThreads.get(),
                1,
                MondrianProperties.instance().SegmentCacheManagerNumberSqlQueueSize.get(),
            ...
            : response.headerMap.entrySet())
        {
            ...
            if (bodyFuture != null) {
                ...
                if (body == SegmentBody.EMPTY) {
                    body = compositeCache.get(header);
                    if (body == null) {
                        // segment cache is expired
                        indexRegistry.getIndex(converter.getRolapStar()).remove(header);
                        return null;
                    }
                    return converter.convert(header, body);
                }
    ...
    public class SegmentCacheIndexRegistry {
        ...
        public final Map<ByteString, SegmentCacheIndex> indexes =
