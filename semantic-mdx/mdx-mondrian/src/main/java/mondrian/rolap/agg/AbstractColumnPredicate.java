...
public abstract class AbstractColumnPredicate implements StarColumnPredicate {
    ...
    public BitKey getConstrainedColumnBitKey() {
        if (constrainedColumnBitKey == null) {
            RolapSchema.PhysColumn physColumn = constrainedColumn.physColumn;
            ArrayList<Integer> sizeAndPositions = new ArrayList<>(2);
            sizeAndPositions.add(physColumn.relation.getSchema().getColumnCount());
            sizeAndPositions.add(physColumn.ordinal());
            constrainedColumnBitKey = BitKey.Factory.getBitKeyWithSizeAndPositions(
                    sizeAndPositions, "Constrained column bit key creation");
        }
