...
public class RolapConnection extends ConnectionBase {
        ...
        DataSource dataSource)
    {
        ...
        if (Objects.nonNull(XmlaRequestContext.getContext())) {
            XmlaRequestContext.getContext().locale = this.locale;
        }
    ...
    private Result executeInternal(final Execution execution) throws Exception {
        execution.setContextMap();
        XmlaRequestContext context = XmlaRequestContext.getContext();
        setMdxContext(context);
        ...
        try {
            ...
            try {
                ...
                // route to other query engine or not
                if (context.useMondrian) {
                    result = executeWithNative(execution, query, context);
                } else {
                    result = executeWithOtherQueryEngine(context.executorClass, execution, context);
                }
            } finally {
                Locus.pop(locus);
                for (RolapStar star : cube.getStars()) {
            ...
            try {
                ...
                context.errorMsg = "MDX Query ID:" + context.runningStatistics.queryID + " Error message:" + rootCause;
                context.runningStatistics.checkException(e);
                RolapUtil.SQL_LOGGER.error(rootCause, e);
                Result result = new RolapResult(execution, true, true, false);
                statement.end(execution);
                return result;
            } catch (Exception e1) {
    ...
    private Result executeWithNative(Execution execution, Query query, XmlaRequestContext context) {
        Result result = new RolapResult(execution, true, false, false);
        int i = 0;
        for (QueryAxis axis : query.getAxes()) {
            if (axis.isNonEmpty()) {
                long createRolapResultStart = System.currentTimeMillis();
                result = new NonEmptyResult(result, execution, i);
                long createRolapResultEnd = System.currentTimeMillis();
                if (context != null) {
                    context.runningStatistics.createRolapResultTime = createRolapResultEnd - createRolapResultStart;
                }
            }
            ++i;
        }
    ...
    private Result executeWithOtherQueryEngine(String executorClass, Execution execution, XmlaRequestContext context) throws Exception {
        MdxExecutor executor = (MdxExecutor) Class.forName(executorClass).newInstance();
        Result result;
        result = executor.execute(execution);
        if (result == null) {
            LOGGER.info(String.format("The query \"%s\" has been handed to Mondrian!", context.mdxQuery));
            context.useMondrian = true;
            String origin = context.mdxQueryRewritten != null ? context.mdxQueryRewritten : context.mdxQuery;
            String mdx = context.mdxOptimizer.rewriteMdx(origin);
            Query query = parseQuery(mdx);
            result = executeWithNative(execution, query, context);
        }
        return result;
    }

    private void setMdxContext(XmlaRequestContext context) {
        if (Objects.isNull(context.queryPage)) {
            // if context.queryPage is null, don't need to set configs about query page to context
            return;
        }
