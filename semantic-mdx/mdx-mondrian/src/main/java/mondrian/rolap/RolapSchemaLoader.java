...
public class RolapSchemaLoader {
    ...
    private final String[] calculatedMemberProperties = {"name", "formatString", "caption", "description", "formula",
            "dimension", "hierarchy", "parent", "visible", "folder"};

    private final String[] calculatedMemberPropertyProperties = {"String name", "caption", "description", "expression", "value"};
    ...
    private boolean hasCustomTranslations(org.eigenbase.xom.NodeDef[] children) {
        for (org.eigenbase.xom.NodeDef child : children) {
            if (child instanceof MondrianDef.CustomTranslations) {
                return true;
            }
        }
        return false;
    ...
    private void loadResource(org.eigenbase.xom.ElementDef element, String tag) {
        org.eigenbase.xom.NodeDef[] children = null;
        Supplier<List> f = null;
        if(element instanceof MondrianDef.Dimension) {
            children = ((MondrianDef.Dimension) element).childArray;
            f = ((MondrianDef.Dimension) element)::getCustomTranslations;
        } else if (element instanceof MondrianDef.Attribute) {
            children = ((MondrianDef.Attribute) element).childArray;
            f = ((MondrianDef.Attribute) element)::getCustomTranslations;
        } else if (element instanceof MondrianDef.Hierarchy) {
            children = ((MondrianDef.Hierarchy) element).childArray;
            f = ((MondrianDef.Hierarchy) element)::getCustomTranslations;
        } else if (element instanceof MondrianDef.Measure) {
            children = ((MondrianDef.Measure) element).childArray;
            f = ((MondrianDef.Measure) element)::getCustomTranslations;
        } else if (element instanceof MondrianDef.CalculatedMember) {
            children = ((MondrianDef.CalculatedMember) element).childArray;
            f = ((MondrianDef.CalculatedMember) element)::getCustomTranslations;
        } else {
            LOGGER.warn("The caption translation of this element is not supported: " + element.getClass());
        }
        if(children != null && f != null) {
            if(hasCustomTranslations(children)) {
                List<MondrianDef.CustomTranslation> xmlCustomTranslations = f.get();
                for(MondrianDef.CustomTranslation xmlCustomTranslation : xmlCustomTranslations) {
                    String name = xmlCustomTranslation.getNameAttribute();
                    String caption = xmlCustomTranslation.getCaption();
                    if (caption != null && !"".equals(caption)) {
                        Locale locale = Util.parseLocale(name);
                        Util.putMulti(resourceMap, tag, new Larders.Resource(
        ...
        MondrianDef.Schema xmlSchema)
    {
        ...
        for (MondrianDef.MeasureGroup xmlMeasureGroup : xmlMeasureGroups) {
                ...
                : xmlMeasureGroup.getMeasures())
            {
                ...
                String measureTable = xmlMeasure.table;
                if (measureTable == null || measureTable.length() == 0) {
                    measureTable = xmlMeasureGroup.table;
                }
                ...
                : xmlMeasureGroup.getDimensionLinks())
            {
                ...
                if (dimension == null) {
                    LOGGER.warn("Dimension \'" + xmlDimensionLink.dimension +
                            "\' not found in this cube");
                    continue;
                }
            ...
            if (xmlMeasureGroup.isM2M != null && xmlMeasureGroup.isM2M) {
                measureGroup.setManytoManyInfo(xmlMeasureGroup.PK, xmlMeasureGroup.bridgeTable);
            }
        }
...
//        }
        ...
        for (RolapMeasureGroup measureGroup : cube.getMeasureGroups()) {
            ...
            for (RolapStoredMeasure measure : measureGroup.measureList) {
                ...
                RolapStar.Table table = star.getFactTable();
                if (expr != null) {
                    if(measureGroup.getFactRelation() != expr.relation) {
                        ...
                        RolapSchema.PhysPathBuilder pathBuilder =
                                new RolapSchema.PhysPathBuilder(table.getRelation());
                        try {
                        ...
                        table = star.getTable(pathBuilder.done());
                    }
                }
            ...
            // all dimension resolved, so we can create a sql from clause
            star.initFromClause();

            // set many to many model
            Pair<String, String> pair = measureGroup.getManyToManyInfo();
            if (pair != null) {
                star.initManyToManyInfo(pair.left, pair.right);
            }
        ...
        Statement statement = createCalcMembersAndNamedSets(
            xmlCube.getCalculatedMembers(),
            xmlCube.getNamedSets(),
            assignDefaultMembers,
            measureList,
        ...
        for (AssignDefaultMember assignDefaultMember : assignDefaultMembers) {
            if (assignDefaultMember instanceof NamedAssignDefaultMember) {
                ((NamedAssignDefaultMember)assignDefaultMember).statement = statement;
            }
        ...
        RolapSchema.PhysPath path)
    {
        ...
        if (attribute.getValueExp() != null) {
            registerExpr(
                    measureGroup, dimension, path, attribute.getValueExp(),
        ...
        final RolapSchema.PhysRelation measureGroupTable)
    {
        ...
        String measureTag = cube.getUniqueName() + "."
                + Util.quoteMdxIdentifier(xmlMeasure.name) + ".measure";
        loadResource(xmlMeasure, measureTag);
        ...
        final NamedList<MondrianDef.Annotation> annotations)
    {
        ...
        String dimensionTag = Util.quoteMdxIdentifier(xmlDimension.name) + ".dimension";
        loadResource(xmlDimension, dimensionTag);
        ...
        for (MondrianDef.Hierarchy xmlHierarchy : xmlDimension.getHierarchies())
        {
            ...
            String hierarchyName = first(
                xmlHierarchy.name,
                xmlDimension.name);
            String hierarchyTag = Util.makeFqName(dimension, hierarchyName)
                    + ".hierarchy";
            loadResource(xmlHierarchy, hierarchyTag);
        ...
        String hierarchyDefaultMember)
    {
        if (hierarchy instanceof RolapCubeHierarchy) {
            ...
            if (hierarchy.getDimension().isMeasures()) {
                ...
                hierarchy.setDefaultMemberExpression(hierarchyDefaultMember);
                hierarchy.setXmlCube(xml);
        ...
        final RolapDimension dimension)
    {
        ...
        RolapSchema.PhysColumn valueExpr =
            createColumn(
                xmlAttribute,
                "valueColumn",
                closureRelation != null
...
//            sb.append(" - ");
        ...
        String dimensionAttributeTag = Util.makeFqName(dimension, xmlAttribute.name)
                + ".attribute";
        loadResource(xmlAttribute, dimensionAttributeTag);

        final RolapAttribute attribute =
            new RolapDimensionAttribute(
                xmlAttribute.name,
                toBoolean(xmlAttribute.visible, true),
                keyList,
                nameExpr,
        ...
        MondrianDef.Level xmlLevel)
    {
        ...
        final RolapAttribute weightAttribute =
                xmlLevel.weightAttribute == null || attribute.getLevelType().name().startsWith("TIME_")
                        ? null
                        : dimension.attributeMap.get(xmlLevel.weightAttribute);
            ...
            new RolapLevel(
                ...
                createLarder(
                    ...
                    xmlLevel.description)
                        .populate(attribute.getLarder()).build(),
                resourceMap);
        ...
        final DataSource dataSource)
    {
        ...
        long hierarchyLoadStartTime = System.currentTimeMillis();
        initializeHierarchiesCache(x);
        XmlaRequestContext context = XmlaRequestContext.getContext();
        if (Objects.isNull(context.runningStatistics.hierarchyLoadTime)) {
            context.runningStatistics.hierarchyLoadTime = 0L;
        }
        context.runningStatistics.hierarchyLoadTime += System.currentTimeMillis() - hierarchyLoadStartTime;
        return x;
    }

    private static void initializeHierarchiesCache(RolapSchema x) {
        HierarchyCache hierarchyCache = CacheManager.getCacheManager().getHierarchyCache();
        RolapCube cube = (RolapCube) x.getCubes()[0];
        XmlaRequestContext.getContext().currentCatalog = cube.getName();
        hierarchyCache.initialize(cube.getHierarchyList());
    }
    ...
    Statement createCalcMembersAndNamedSets(
        ...
        boolean errOnDups)
    {
        List<NamedAssignDefaultMember> namedAssignDefaultMembers = assignDefaultMembers.stream()
                .filter(d -> d instanceof NamedAssignDefaultMember)
                .map(d -> (NamedAssignDefaultMember)d)
                .collect(Collectors.toList());

        final Query queryExp =
            resolveCalcMembers(
                xmlCalcMembers, xmlNamedSets, namedAssignDefaultMembers, cube, errOnDups);
        if (queryExp == null) {
            return null;
        }
        ...
        Util.assertTrue(
            ...
            == xmlCalcMembers.size()
                    + namedAssignDefaultMembers.size()
                    + xmlNamedSets.size());
        ...
        int flexibleDefaultMembersCount = 0;
        for (NamedAssignDefaultMember assignDefaultMember : namedAssignDefaultMembers) {
            Formula formula = queryExp.getFormulas()[xmlCalcMembers.size() + flexibleDefaultMembersCount++];
            assignDefaultMember.exp = formula.getExpression();
        }

        for (int i = 0; i < xmlNamedSets.size(); i++) {
            postNamedSet(
                cube,
                xmlNamedSets, xmlCalcMembers.size() + flexibleDefaultMembersCount, i, queryExp, namedSetList);
        }

        return queryExp.getStatement();
    }

    private Query resolveCalcMembers(
        List<MondrianDef.CalculatedMember> xmlCalcMembers,
        List<MondrianDef.NamedSet> xmlNamedSets,
        List<NamedAssignDefaultMember> namedAssignDefaultMembers,
        RolapCube cube,
        boolean errOnDups)
    {
        // If there are no objects to create, our generated SQL will be so
        // silly, the parser will laugh.
        if (xmlCalcMembers.size() == 0 && xmlNamedSets.size() == 0 && namedAssignDefaultMembers.isEmpty()) {
            return null;
        }
        ...
        for (NamedAssignDefaultMember assignDefaultMember : namedAssignDefaultMembers) {
            buf.append("MEMBER ")
                    .append("[Measures].[Measures].[#DefaultMemberCalculator_")
                    .append(++flexibleDefaultMembersCount)
                    .append(']')
                    .append(Util.nl)
                    .append(" AS ");

            Util.singleQuoteString(assignDefaultMember.defaultMemberName, buf);
            buf.append(",")
                    .append(Util.nl);
        ...
        List<Formula> formulaList)
    {
        ...
        namedSet.setVisible(toBoolean(xmlNamedSet.visible, true));
        ...
        List<RolapMember> memberList)
    {
        ...
        String memberTag = cube + "." + member + ".member";
        loadResource(xmlCalcMember, memberTag);
        ((RolapCalculatedMember) member).setLarder(
            Larders.LarderBuilder.of(member.getLarder())
                ...
                .addAll(resourceMap.get(memberTag))
                .add(Property.VISIBLE, toBoolean(xmlCalcMember.visible, true))
                .build());

        if (member instanceof RolapHierarchy.RolapCalculatedMeasure) {
            NamedList<MondrianDef.NonEmptyBehaviorMeasure> nonEmptyBehaviorMeasures = xmlCalcMember.getNonEmptyMeasures();
            if (nonEmptyBehaviorMeasures != null) {
                for (MondrianDef.NonEmptyBehaviorMeasure nonEmptyBehaviorMeasure : nonEmptyBehaviorMeasures) {
                    String measureName = nonEmptyBehaviorMeasure.getNameAttribute();
                    measureList.stream()
                            .filter(measure -> measure.getName().equals(measureName))
                            .findFirst()
                            .ifPresent(((RolapHierarchy.RolapCalculatedMeasure)member)::addNonEmptyBehaviourMeasure);
                }
            }
        }
    ...
    class PrintXmlObjectForTroubshooting extends RuntimeException {
        public PrintXmlObjectForTroubshooting(String message, Throwable cause) {
            super(message, cause);
        }
    }

    public void addObjBuf(Object obj, StringBuilder buf, int preLen) {
        StringBuilder sb = new StringBuilder();
        int p = preLen;
        while(p-- != 0) {
            sb.append(" ");
        ...
        for (Field field : fields)
        {
            ...
            try
            {
                ...
                if ("_def".equals(name) || "children".equals(name) || "EMPTY".equals(name)) {
                    continue;
                }
                Object value = field.get(obj);
                if (buf.length() == 0 || (!Util.nl.equals(buf.charAt(buf.length()-1)))) {
                        buf.append(Util.nl);
                }

                buf.append(pre).append(name).append("=");
                if (value instanceof Object[]) {
                    ...
                    for (int i = 0; i < v.length; i++) {
                        if (i > 0) {
                            buf.append(Util.nl).append(sb2.toString()).append(",");
                        }
                        addObjBuf(v[i], buf, preLen + name.length() + 1);
                    }
                } else if (value instanceof org.eigenbase.xom.ElementDef) {
                    addObjBuf(value, buf, preLen + name.length() + 1);
                } else {
                    buf.append(value);
                }
            }
    ...
    public RuntimeException getPrintObjectEx(Object obj, String[] properties, Throwable cause) {
        String str = null;
        if (obj instanceof org.eigenbase.xom.ElementDef) {
            str = obj.toString();
        } else {
            StringBuilder buf = new StringBuilder();
            addObjBuf(obj, buf, 0);
            if (buf.length() > 0) {
                str = obj + buf.toString();
            }
        }

        if (str != null) {
            return new PrintXmlObjectForTroubshooting(str, cause);
        } else {
        ...
        Set<String> fqNames)
    {
        ...
        {
            throw getPrintObjectEx(xmlCalcMember, calculatedMemberProperties, MondrianResource.instance()
        ...
        if (hierarchy == null) {
            throw getPrintObjectEx(xmlCalcMember, calculatedMemberProperties, MondrianResource.instance().CalcMemberHasBadDimension.ex(
        ...
        for (int i = 0; i < calculatedMemberList.size(); i++) {
            ...
            {
                if (errOnDup) {
                    throw getPrintObjectEx(xmlCalcMember, calculatedMemberProperties, MondrianResource.instance().CalcMemberNotUnique.ex(
                        fqName,
                        cube.getName()));
                } else {
        ...
        processDisplayFolder(xmlCalcMember, buf);
    ...
    private void processDisplayFolder(MondrianDef.CalculatedMember xmlCalcMember, StringBuilder buf) {
        if (xmlCalcMember.folder != null && xmlCalcMember.folder.trim().length() != 0) {
            buf.append(",")
                    .append(Util.nl)
                    .append(Property.DISPLAY_FOLDER.name)
                    .append(" = ")
                    .append(Util.quoteForMdx(xmlCalcMember.folder));
        }
    }
        ...
        String memberName)
    {
        ...
        for (MondrianDef.CalculatedMemberProperty xmlProperty : xmlProperties) {
            if (xmlProperty.expression == null && xmlProperty.value == null) {
                throw getPrintObjectEx(xmlProperty, calculatedMemberPropertyProperties, MondrianResource.instance()
                    .NeitherExprNorValueForCalcMemberProperty.ex(
                        xmlProperty.name, memberName, cube.getName()));
            }
            if (xmlProperty.expression != null && xmlProperty.value != null) {
                throw getPrintObjectEx(xmlProperty, calculatedMemberPropertyProperties, MondrianResource.instance().ExprAndValueForMemberProperty
                    .ex(
                        xmlProperty.name, memberName, cube.getName()));
            }
    ...
    public void updateDefaultMembers(RolapCube cube) {
        assignDefaultMembers.clear();

        for (RolapCubeHierarchy cubeHierarchy : cube.getHierarchyList()) {
            String defaultMemberExpression = cubeHierarchy.getRolapHierarchy().getDefaultMemberExpression();
            ElementDef xmlCube = cubeHierarchy.getRolapHierarchy().getXmlCube();
            if (defaultMemberExpression == null || xmlCube == null) {
                continue;
            }
            assignDefaultMembers.add(new NamedUpdatingAssignDefaultMember(cube, cubeHierarchy, xmlCube, defaultMemberExpression));
        }
    ...
    private class NamedAssignDefaultMember extends AssignDefaultMember {
            ...
            String defaultMemberName)
        {
            ...
            List<Id.Segment> tempUniqueNameParts;
            if (defaultMemberName.contains("[")) {
                try {
                    tempUniqueNameParts = Util.parseIdentifier(defaultMemberName);
                } catch (IllegalArgumentException e) {
                    LOGGER.info(String.format("Constant Default Member parsing failed, maybe encountered an dynamic default member: \"%s\".", defaultMemberName));
                    tempUniqueNameParts =
            ...
            uniqueNameParts = tempUniqueNameParts;
        }
        ...
        public void apply2() {
            ...
            for (RolapCubeHierarchy cubeHierarchy : cubeHierarchies()) {
                ...
                if (member == null) {
                    ...
                    Locus locus = new Locus(execution, null, "Calculating default members");
                    Locus.push(locus);
                    RolapResult rolapResult = new RolapResult(execution, true, false, true);

                    try {
                        Calc calc = rolapResult.evaluator.root.getCompiled(exp, false, ResultStyle.ITERABLE);
                        Object result = rolapResult.evaluateExp(calc, rolapResult.slicerEvaluator);

                        if (!(result instanceof RolapMember)) {
                            LOGGER.error(String.format(
                                    "The value of Hierarchy %s's Default Member \"expression\" %s is not a member.",
                                    cubeHierarchy.getUniqueName(),
                                    defaultMemberName));
                            failureCount++;
                            continue;
                        }
                        member = (RolapMember)result;
                        if (!member.getHierarchy().equals(cubeHierarchy)) {
                            LOGGER.error(String.format(
                                    "The value of Hierarchy %s's Default Member \"expression\" %s belongs to another Hierarchy %s.",
                                    cubeHierarchy.getUniqueName(),
                                    defaultMemberName,
        ...
        protected void fail() {
            handler.error(
                "Can not find Default Member with name or expression \""
        ...
        protected RolapMember lookup(
            ...
            RolapCubeHierarchy cubeHierarchy)
        {
            RolapCube cube = cubeHierarchy.getCube();
    ...
    private class NamedUpdatingAssignDefaultMember extends NamedAssignDefaultMember {
        private final RolapCubeHierarchy cubeHierarchy;
        public NamedUpdatingAssignDefaultMember(RolapCube cube, RolapCubeHierarchy cubeHierarchy, ElementDef xml, String defaultMemberName) {
            super(cube, cubeHierarchy.getRolapHierarchy(), xml, defaultMemberName);
            this.cubeHierarchy = cubeHierarchy;
        }

        @Override
        protected List<RolapCubeHierarchy> cubeHierarchies() {
            return Collections.singletonList(cubeHierarchy);
        }
    }
}
