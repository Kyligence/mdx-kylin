...
public class RolapHierarchy extends HierarchyBase {
    ...
    private String defaultMemberExpression;
    private ElementDef xmlCube;
        ...
        Larder larder)
    {
        ...
        if (attribute instanceof RolapDimensionAttribute) {
            subfolder = ((RolapDimensionAttribute) attribute).subfolder;
        } else {
            subfolder = "";
        }
    }
...
*/
    ...
    public boolean isCustomRollup() {
        if (customRollup == null) {
            customRollup = levelList.stream()
                    .anyMatch(level -> level.getWeightAttribute() != null);
        }
        return customRollup;
    }

    public String getDefaultMemberExpression() {
        return defaultMemberExpression;
    }

    public void setDefaultMemberExpression(String defaultMemberExpression) {
        this.defaultMemberExpression = defaultMemberExpression;
    }

    public ElementDef getXmlCube() {
        return xmlCube;
    }

    public void setXmlCube(ElementDef xmlCube) {
     ...
     * [Measures] dimension).
     */
    public static class RolapCalculatedMeasure
        extends RolapCalculatedMember
        implements RolapMeasure
    {
        private ValueFormatter cellFormatter;

        private List<RolapMember> nonEmptyBehaviourMeasures;
                ...
                try {
                    CellFormatter formatter =
                        RolapSchemaLoader.getFormatter(
                            ...
                            null);
                    this.cellFormatter =
                        new CellFormatterValueFormatter(formatter);
                } catch (Exception e) {
        ...
        public void addNonEmptyBehaviourMeasure(RolapMember nonEmptyBehaviourMeasure) {
            if (nonEmptyBehaviourMeasures == null) {
                nonEmptyBehaviourMeasures = new ArrayList<>();
            }
            nonEmptyBehaviourMeasures.add(nonEmptyBehaviourMeasure);
        }

        public List<RolapMember> getNonEmptyBehaviourMeasures() {
            return nonEmptyBehaviourMeasures;
        }
    }
