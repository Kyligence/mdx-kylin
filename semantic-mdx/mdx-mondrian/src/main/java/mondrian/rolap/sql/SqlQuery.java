...
public class SqlQuery {
    private static final Pattern ORDER_INDICATOR_PATTERN = Pattern.compile("( (ASC)|(DESC))$");

    /** Controls the formatting of the sql string. */
    protected final boolean generateFormattedSql;

    protected boolean distinct;
    protected final ClauseList select;
    protected FromClauseList from;
    protected final ClauseList where;
    protected final ClauseList groupBy;
    protected final ClauseList having;
    protected ClauseList orderBy;
    protected final List<ClauseList> groupingSets;
    protected final ClauseList groupingFunctions;

    protected final List<SqlStatement.Type> types =
     ...
     */
    protected final List<String> fromAliases;
    ...
    protected final List<String> columnAliases2 = new ArrayList<String>();

    protected static final String INDENT = "    ";

    protected boolean cellCalc;
    ...
    public boolean isCellCalc() {
        return cellCalc;
    }

    public void setCellCalc(boolean cellCalc) {
        this.cellCalc = cellCalc;
    }
...
*/
    ...
    public void setOrderAscending(boolean ascending) {
        for (int i = 0; i < orderBy.size(); i++) {
            String orderByClause = orderBy.get(i);
            Matcher matcher = ORDER_INDICATOR_PATTERN.matcher(orderByClause);
            if (matcher.find()) {
                orderBy.set(i, matcher.replaceAll(ascending ? " ASC" : " DESC"));
            }
        }
    }
    ...
    public StringBuilder toBuffer(StringBuilder buf, String prefix) {
        ...
        if (hasLimit)
            buf.append(" limit " + MondrianProperties.instance().MaxPullSize.get());
        return buf;
    }
    ...
    protected static class FromClauseList extends ClauseList {
        private final List<JoinOnClause> joinOnClauses =
            new ArrayList<JoinOnClause>();

        protected FromClauseList(boolean allowsDups) {
            super(allowsDups);
        }
    ...
    protected static class ClauseList extends ArrayList<String> {
            ...
            String s)
        {
            if (generateFormattedSql) {
                if (s.startsWith(" ")) {
                    // E.g. " and "
                    s = Util.getNlBySqlFormatted() + prefix + s.substring(1);
                }
                if (s.endsWith(" ")) {
                    // E.g. ", "
                    s = s.substring(0, s.length() - 1) + Util.getNlBySqlFormatted() + prefix
                        +  INDENT;
                } else if (s.endsWith("(")) {
                    // E.g. "("
                    s = s + Util.getNlBySqlFormatted() + prefix + INDENT;
                }
            }
            return s;
        }
