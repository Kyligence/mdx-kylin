...
public class CrossJoinArgFactory {
        ...
        final boolean returnAny)
    {
        ...
        cjArgs = checkDescendants(evaluator, role, fun, args);
        if (cjArgs != null) {
            return Collections.singletonList(cjArgs);
        }
        ...
        Exp[] args)
    {
        ...
        return new CrossJoinArg[]{
            new DescendantsCrossJoinArg(level, member, false)
        };
    }
        ...
        Exp[] args)
    {
        ...
        return new CrossJoinArg[]{
            new DescendantsCrossJoinArg(level, null, false)
        };
    }
        ...
        int argSize)
    {
        ...
        if (argSize > MondrianProperties.instance().FilterPushdownInClauseMaxSize.get()) {
    ...
    /**
     * Checks for Descendants(&lt;member&gt;, &lt;Level&gt;) or
     ...
     * something else.
     */
    public CrossJoinArg[] checkDescendants(
        Evaluator evaluator,
        Role role,
        FunDef fun,
        Exp[] args)
    {
        ...
        if (args.length < 2
                || args.length > 3
                || args.length == 3
                && !(args[1].getType() instanceof EmptyType
                && args[2] instanceof Literal
                && ((String) ((Literal) args[2]).getValue()).equalsIgnoreCase("LEAVES"))) {
            return null;
        }

        final RolapMember member;
        if (args[0] instanceof MemberExpr) {
            member = (RolapMember) ((MemberExpr) args[0]).getMember();
        } else if (args[0] instanceof ResolvedFunCall
                && ((ResolvedFunCall) args[0]).getFunName().equals("CurrentMember")) {
            member = (RolapMember) evaluator.getContext(args[0].getType().getHierarchy());
        } else {
            return null;
        }
        if (member.isCalculated()
        ...
        if ((args[1] instanceof LevelExpr)) {
            ...
            List<? extends RolapCubeLevel> levels = member.getHierarchy().getLevelList();
        ...
        } else if (args[1].getType() instanceof EmptyType) {
        ...
        return new CrossJoinArg[]{
            new DescendantsCrossJoinArg(level, member, leaves)
        };
    }
