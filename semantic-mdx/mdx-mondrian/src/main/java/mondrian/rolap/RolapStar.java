...
public class RolapStar {
    ...
    private String factPk;

    private String bridgeTable;

    private final Table factTable;

    private static final int DEFAULT_CARDINALITY = MondrianProperties.instance().FilterPushdownInClauseMaxSize.get() * 2;
    ...
    /**
     * Keeps every used link of physical model, contains bridge link (snow-flake)
     */
    private Set<List<RolapSchema.PhysLink>> links = new HashSet<>();

    private Map<String, RolapSchema.PhysPath> colPathMap = new HashMap<>();
    ...
    private final LoadingCache<BitKey, Column[]> cachedColumns;
        ...
        final RolapSchema.PhysRelation fact)
    {
        ...
        CacheLoader<BitKey, Column[]> columnsLoader = new CacheLoader<BitKey, Column[]>() {
            @Override
            public Column[] load(BitKey key) throws Exception {
                return createColumnsFromBitKey(key);
            }
        };
        this.cachedColumns = CacheBuilder.newBuilder()
                .softValues()
                .build(columnsLoader);
    }
    ...
    public void initFromClause() {
        Set<RolapSchema.PhysPath> pathSet = new HashSet<>();
        for(Column column : this.columnList) {
            pathSet.add(column.getTable().path);
            RolapSchema.PhysColumn expression = column.getExpression();
            if (expression != null) {
                colPathMap.put(expression.toSql(), column.getTable().path);
            }
        }
        for (RolapSchema.PhysPath path : pathSet) {
    ...
    public void initManyToManyInfo(String pk, String bridgeTable) {
        this.factPk = pk;
        this.bridgeTable = bridgeTable;
    }

    public String getManyToManyPk() {
        return this.factPk;
    }
    ...
    public Pair<Table, Map<String, RolapSchema.PhysPath>> getFactAndPath() {
        return new Pair<Table, Map<String, RolapSchema.PhysPath>>(factTable, colPathMap);
    }
    ...
    public static class Bar {
        ...
        public boolean isEmpty() {
            return aggregations.isEmpty() && segmentRefs.isEmpty();
        }
    }
    ...
    public Column[] createColumnsFromBitKey(BitKey key) {
        Column[] columnsCache = new Column[key.cardinality()];
        int i = 0;
        for (int bitPos = key.nextSetBit(0);
             bitPos >= 0;
             bitPos = key.nextSetBit(bitPos + 1))
        {
            columnsCache[i] = getColumn(bitPos);
            i++;
        }
        return columnsCache;
    }

    public Column[] getColumnsFromBitKey(BitKey key, String component) {
        try {
            return cachedColumns.get(key);
        } catch (ExecutionException e) {
            LOGGER.warn(component + ": Fetching cached star columns failed.", e);
            return createColumnsFromBitKey(key);
        }
    }
...
//                   || internalType == expression.getInternalType();
    ...
    public static class Measure extends Column {
        ...
        public boolean isSum() {
            String aggregator = getAggregator().getName();
            return "sum".equals(aggregator);
        }

        public boolean isM2MAccumulative() {
            if (!getStar().isManyToMany()) {
                return false;
            }
            RolapSchema.PhysRelation table = getTable().getRelation();
            String measureTable;
            if (table instanceof RolapSchema.PhysTable) {
                RolapSchema.PhysTable table0 = (RolapSchema.PhysTable) table;
                measureTable = table0.name;
            } else {
                measureTable = getTable().getTableName() ;
            }
            String bridgeTable = getStar().getBridgeTable();
            return isAccumulative() && !measureTable.equals(bridgeTable);
        }

        private boolean isAccumulative() {
            String aggregator = getAggregator().getName();
            return "sum".equals(aggregator) || "count".equals(aggregator);
        }
    }
            ...
            boolean rollup)
        {
            ...
            RolapStar.Measure starMeasure =
                new RolapStar.Measure(
                    ...
                    measure.getMeasureGroup().getName(),
                    rollup
