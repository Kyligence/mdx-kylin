...
public class RolapCubeLevel extends RolapLevel {
    ...
    void initLevel(RolapSchemaLoader schemaLoader) {
        ...
        if (MondrianProperties.instance().InitGroupByOrderByKeyForHierarchyLevel.get()) {
            initGroupByAndOrderByKeys();
        }
    }

    private void initGroupByAndOrderByKeys() {
        if (isAll() || this.depth <= 1) {
            return;
        }
        List<RolapSchema.PhysColumn> keyCols = new ArrayList<>(4);
        for (int i = 1; i < this.depth; i++) {
            RolapCubeLevel parentLevel = this.getHierarchy().getLevelList().get(i);
            RolapSchema.PhysColumn parentCol = parentLevel.getAttribute().getKeyList().get(i - 1);
            keyCols.add(parentCol);
        }
        keyCols.add(this.attribute.getKeyList().get(0));
        this.attribute = this.attribute.cloneWithNewKeyCols(keyCols, keyCols);
    }
...
*/
    ...
    {
        if (name instanceof Id.KeySegment && attribute.getKeyList().size() == 1) {
            ...
            if (keyExps.size() == 1 && schemaReader instanceof RolapSchemaReader) {
                RolapMember allMember = cubeHierarchy.getAllMember();
                RolapSchemaReader rolapSchemaReader = (RolapSchemaReader) schemaReader;
                if (rolapSchemaReader.skipValidateFromSource(MatchType.EXACT, allMember, name)) {
                    return ((RolapSchemaReader) schemaReader).makeExactChildMember(allMember, name);
                }
            }
    ...
    static final class RegularLevelReader implements LevelReader {
                ...
                CellRequest request)
        {
            ...
            List<RolapSchema.PhysColumn> keyList = cubeLevel.attribute.getKeyList();
            StarPredicate cachedPredicate = null;
            if (!keyList.isEmpty()) {
                cachedPredicate = measureGroup.getStarPredicate(member, "Compound predicate creation");
            }
            for (int i = 0; i < keyList.size(); i++) {
                RolapSchema.PhysColumn column = keyList.get(i);
                RolapStar.Column starColumn = measureGroup.getRolapStarColumn(
                ...
                if (starColumn == null) {
                    ...
                    // if it is a no link dimension, we will show the value non-group by this dimension
                    if (!measureGroup.dimensionMap3.containsKey(cubeLevel.getDimension())) {
                        return false;
                    } else {
                ...
                if (member.isCalculated() && !member.isParentChildLeaf()) {
                    ...
                    if (cachedPredicate instanceof ValueColumnPredicate) {
                        assert i == 0;
                        predicate = (ValueColumnPredicate)cachedPredicate;
                    } else {
                        predicate = (ValueColumnPredicate)((AndPredicate)cachedPredicate).getChildren().get(i);
                    }
                }
