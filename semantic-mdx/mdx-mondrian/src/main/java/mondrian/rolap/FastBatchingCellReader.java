...
//    private int pendingCount;
    ...
    public FastBatchingCellReader(
        ...
        cellRequestLimit = MondrianProperties.instance().CellBatchSize.get() <= 0
                ? MondrianProperties.instance().ResultLimit.get() // TODO Make this logic into a pluggable algorithm.
                : MondrianProperties.instance().CellBatchSize.get();
    }

    public int getCellRequestLimit() {
        return cellRequestLimit;
    }
    ...
    public Object get(RolapEvaluator evaluator) {
        ...
        return getCellValue(request);
    }

    private Object getCellValue(CellRequest request) {
        if (request == null || request.isUnsatisfiable()) {
            return Util.nullValue; // request not satisfiable.
        }

        // Try to retrieve a cell and simultaneously pin the segment which contains it.
        ...
        boolean isFromSmartBI = Objects.nonNull(context) && context.clientType.equals(XmlaRequestContext.ClientType.SMARTBI);
        if (cacheEnabled && missCount == 0 && !isFromSmartBI) {
            SegmentWithData segmentWithData = cacheMgr.peek(request);
            if (Objects.nonNull(segmentWithData)) {
                segmentWithData.getStar().register(segmentWithData);
                final Object o2 = aggMgr.getCellFromCache(request, pinnedSegments);
                if (o2 != null) {
                    ++hitCount;
                    return o2;
                }
        ...
        // if there is no such cell, record that we need to fetch it, and return 'error'
    ...
    public final void recordCellRequest(CellRequest request) {
        ...
        if (cellRequestLimit != 0 && cellRequests.size() % cellRequestLimit == 0) {
    ...
    public boolean loadAggregations() {
        ...
        final List<Future<Map<Segment, SegmentWithData>>> sqlSegmentMapFutures = new ArrayList<>();

        final List<CellRequest> nowCellRequests = new ArrayList<>(cellRequests);
        ...
        for (int iteration = 0; ; ++iteration) {
            final BatchLoader.LoadBatchResponse response =
                    cacheMgr.execute(
                            new BatchLoader.LoadBatchCommand(
                                    ...
                                    getDialect(),
                                    cube,
                                    Collections.unmodifiableList(nowCellRequests),
                                    XmlaRequestContext.getContext()));

            int failureCount = 0;

            // Segments that have been retrieved from cache this cycle. Allows
            // us to reduce calls to the external cache.
            Map<SegmentHeader, SegmentBody> headerBodies = new HashMap<>();
            ...
            final Map<SegmentHeader, SegmentBody> succeededRollups = new HashMap<>();
            ...
            if (failureCount == 0 || iteration > 0) {
                ...
                for (Map.Entry<SegmentHeader, Future<SegmentBody>> entry
                    ...
                    SegmentBody body = Util.safeGet(
                            bodyFuture,
                            "Waiting for someone else's segment to load via SQL");
                    if (body == SegmentBody.EMPTY) {
                        body = cacheMgr.compositeCache.get(header);
                    }
                    if (body == null) {
                        SegmentBuilder.SegmentConverter converter = response.converterMap
                                .get(SegmentCacheIndexImpl.makeConverterKey(header));
                        cacheMgr.getIndexRegistry().getIndex(converter.getRolapStar()).remove(header);
                        continue;
                    }
                    final SegmentWithData segmentWithData =
                            response.convert(header, body);
            ...
            List<CellRequest> old = new ArrayList<>(nowCellRequests);
            nowCellRequests.clear();
            for (CellRequest cellRequest : old) {
                if (cellRequest.getMeasure().getStar().getCellFromCache(cellRequest, null) == null) {
                    nowCellRequests.add(cellRequest);
                }
            }

            if (nowCellRequests.isEmpty()) {
                break;
            }

            if (nowCellRequests.size() >= old.size()
                    && iteration > 10) {
                throw Util.newError(
                        "Cache round-trip did not resolve any cell requests. "
                                + "Iteration #" + iteration
                                + "; request count " + nowCellRequests.size()
                                + "; requested headers: " + response.cacheSegments.size()
                                + "; requested rollups: " + response.rollups.size()
                                + "; requested SQL: " + response.sqlSegmentMapFutures.size());
            }
...
//        }
    ...
    private boolean loadFromCaches(
        ...
        final List<SegmentHeader> headersInCache =
                index.locate(
                        ...
                        mappedCellValues,
                        compoundPredicates,
                        request.getCustomRollupRequirements());
        ...
        boolean isManyToMany = star.isManyToMany();
    ...
    private boolean headerCoversRequest(
        ...
        for (int i = 0; i < request.getConstrainedColumns().length; i++) {
            ...
            if (headerValues.get(valueIdx) != null
                    ...
                    ((ValueColumnPredicate)request.getValueAt(i)).getValue())) {
                return false;
            }
        }
        return true;
    ...
    private void loadFromSql(
        ...
        if (batch == null) {
            if (MondrianProperties.instance().EnableOtherCMCalculationDetailsLog.get()) {
                String info = "New batch for key with content: " + key.toString();
                LOGGER.info(info);
            }
    ...
    public class Batch implements Loadable {
        ...
        private final CustomRollupRequirements customRollupRequirements;
        ...
        final void loadAggregation(
            ...
            // Handle Custom Rollup measures
            if (customRollupRequirements != null) {
                List<RolapStar.Measure> sumMeasures = measuresList.stream()
                        .filter(RolapStar.Measure::isSum)
                        .collect(Collectors.toList());
                for (RolapStar.Measure measure : sumMeasures) {
                    AggregationManager.loadAggregation(
                            starConverter,
                            cacheMgr,
                            cellRequestCount,
        ...
        private void generateAggregateSql() {
            if (cube == null || cube.isVirtual()) {
                ...
                final AggGen aggGen =
                        new AggGen(
                                cube.getName(),
                                cube.getMeasureGroups().get(0).getStar(),
                                columns);
                if (aggGen.isReady()) {
