        ...
        extends Serializable, Comparable<BitKey>, Iterable<Integer>
{
    ...
    static BitKey unmodifiableBitKey(BitKey bitKey) {
        return bitKey instanceof UnmodifiableBitKey ? bitKey : new UnmodifiableBitKey(bitKey);
    }
    ...
    public abstract class Factory {
        /**
         * key[0]: BitKey size<br>
         * key[1:]: All true positions<br>
         */
        private static final LoadingCache<List<Integer>, BitKey> cachedBitKeys;

        static {
            final CacheLoader<List<Integer>, BitKey> bitKeysLoader = new CacheLoader<List<Integer>, BitKey>() {
                @Override
                public BitKey load(List<Integer> key) {
                    return createBitKeyWithSizeAndPositions(key);
                }
            };
            cachedBitKeys = CacheBuilder.newBuilder()
                    .softValues()
                    .build(bitKeysLoader);
        }

        public static BitKey createBitKeyWithSizeAndPositions(List<Integer> sizeAndPositions) {
            BitKey bitKey = makeBitKey(sizeAndPositions.get(0));
            for (int i = 1; i < sizeAndPositions.size(); i++) {
                bitKey.set(sizeAndPositions.get(i));
            }
            return BitKey.unmodifiableBitKey(bitKey);
        }

        public static BitKey getBitKeyWithSizeAndPositions(List<Integer> sizeAndPositions, String component) {
            try {
                return cachedBitKeys.get(sizeAndPositions);
            } catch (ExecutionException e) {
                log.warn(component + ": Fetching cached BitKey instance failed.", e);
                return createBitKeyWithSizeAndPositions(sizeAndPositions);
            }
        }
    ...
    class UnmodifiableBitKey implements BitKey {
        private final BitKey bitKey;

        private UnmodifiableBitKey(BitKey bitKey) {
            this.bitKey = bitKey;
        }

        @Override
        public String toBinaryString() {
            return bitKey.toBinaryString();
        }
        ...
        public boolean isSuperSetOf(BitKey bitKey) {
            return this.bitKey.isSuperSetOf(bitKey);
        }

        @Override
        public BitKey or(BitKey bitKey) {
            return this.bitKey.or(bitKey);
        }

        @Override
        public BitKey orNot(BitKey bitKey) {
            return this.bitKey.orNot(bitKey);
        }

        @Override
        public BitKey and(BitKey bitKey) {
            return this.bitKey.and(bitKey);
        }

        @Override
        public BitKey andNot(BitKey bitKey) {
            return this.bitKey.andNot(bitKey);
        }
        ...
        public BitKey emptyCopy() {
            return bitKey.emptyCopy();
        }

        @Override
        public boolean isEmpty() {
            return bitKey.isEmpty();
        }

        @Override
        public boolean intersects(BitKey bitKey) {
            return this.bitKey.intersects(bitKey);
        }

        @Override
        public BitSet toBitSet() {
            return bitKey.toBitSet();
        }

        @Override
        public Iterator<Integer> iterator() {
            return bitKey.iterator();
        }

        @Override
        public int nextSetBit(int fromIndex) {
            return bitKey.nextSetBit(fromIndex);
        }

        @Override
        public int cardinality() {
            return bitKey.cardinality();
        }

        @Override
        public int compareTo(BitKey o) {
            o = undelegated(o);
            return bitKey.compareTo(o);
        }
        ...
        public boolean equals(Object o) {
            ...
            o = undelegated((BitKey)o);
            return bitKey.equals(o);
        }

        @Override
        public int hashCode() {
            return bitKey.hashCode();
        }
    }

    static BitKey undelegated(BitKey bitKey) {
        while (bitKey instanceof UnmodifiableBitKey) {
            bitKey = ((UnmodifiableBitKey)bitKey).bitKey;
        }
        return bitKey;
    }
