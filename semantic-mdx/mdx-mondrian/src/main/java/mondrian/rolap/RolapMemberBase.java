    ...
    implements RolapMember
{
    ...
    private static final boolean memberKeyCanBeList = MondrianProperties.instance().MemberKeyCanBeList.get();
    ...
    public RolapMemberBase(
        ...
        Larder larder)
    {
        this(parentMember, level, key, null, memberType, uniqueName, larder);
    }
    ...
    public boolean isVisible() {
        Object visible = larder.get(Property.VISIBLE);
        ...
        boolean calc)
    {
        ...
        OlapElement deriveElement = deriveRootUniqueName(parentMember, level, name, calc);
        if (deriveElement instanceof RolapHierarchy && !level.isAll()) {
            RolapHierarchy hierarchyElement = (RolapHierarchy) deriveElement;
            if (hierarchyElement.getLevelList().size() == 2) {
                deriveElement = hierarchyElement;
            } else {
                deriveElement = hierarchyElement.getLevelList().get(1);
            }
            return Util.makeFqNameWithAnd(deriveElement, name);
        }

        if (deriveElement instanceof RolapCubeLevel) {
            return Util.makeFqNameWithAnd(deriveElement, name);
        }

        if (deriveElement instanceof RolapMember) {
            return Util.makeFqNameWithAnd(deriveElement, name);
        }

        return Util.makeFqName(deriveElement, name);
    }
    ...
    public Object getPropertyValue(Property property) {
        if (property != null) {
            ...
            case Property.MEMBER_KEY_ORDINAL:
                ...
                // Only return the last element if the key is a list.
                Comparable<?> key = getKey();
                if (memberKeyCanBeList) {
                    return key;
                }
                ...
                if (key instanceof List<?>) {
                    ...
                    Object lastKey = keyList.get(keyList.size() - 1);
                    if (lastKey instanceof Comparable<?>) {
                        return lastKey;
                    }
                }
                return key;
    ...
    public Comparable getOrderKey() {
        ...
        if (key instanceof List) {
            return (Comparable) ((List) key).get(((List) key).size() - 1);
        }
        return key;
    }
