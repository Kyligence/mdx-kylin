        ...
        NameResolver.Namespace
{
        ...
        Member _parent, Id.Segment childName, MatchType matchType)
    {
        ...
        try {
            ...
            if (skipValidateFromSource(matchType, parent, childName)) {
                RolapMemberBase madeChildMember = makeExactChildMember(parent, childName);
                if (madeChildMember != null) {
                    children = Arrays.asList(madeChildMember);
                } else {
    ...
    public RolapMemberBase makeExactChildMember(RolapMember parent, Id.Segment childName) {
        if (NameResolver.segmentsStore.get() != null) {
            List<IdentifierSegment> segments = NameResolver.segmentsStore.get();
            String[] parentSegments = parent.getHierarchy().getUniqueName().split("\\.");
            for (int i = 0; i < parentSegments.length; i++) {
                if (!parentSegments[i].contentEquals(segments.get(i).toString())) {
                    return null;
                }
            }
        }
        ...
        try {
            ...
            if (childName instanceof Id.NameSegment) {
                nameValue = ((Id.NameSegment) childName).getName();
            }
            if (childName instanceof Id.KeySegment) {
                // TODO, support compound key
                nameValue = childName.getKeyParts().get(0).getName();
            }
            ...
            SqlStatement.Type sqlType = currentLevel.getOrderByList().get(0).getInternalType();
            Comparable keyValue = makeKeyValue(nameValue, sqlType);
            Comparable memberKey = null;
            ...
            LOGGER.error(String.format("Invalid name \"%s\" !", childName));
            return null;
        }
    }

    private Comparable makeKeyValue(String nameValue, SqlStatement.Type sqlType) {
        if (RolapUtil.NULL_NAME.equals(nameValue)) {
        ...
        switch (sqlType) {
            ...
            case DATE:
                return java.sql.Date.valueOf(nameValue);
            case TIMESTAMP:
                return Timestamp.valueOf(nameValue);
            case OBJECT:
            case STRING:
            default:
                return nameValue;
    ...
    public boolean skipValidateFromSource(MatchType matchType, RolapMember parent, Id.Segment segment) {
        // if key column and name column is not the same, should not skip validate
        RolapCubeLevel curLevel = parent.getLevel().getChildLevel();
        if (curLevel == null || curLevel.getInheritedProperties().length > 0) {
            return false;
        }
        List<RolapSchema.PhysColumn> keyList = curLevel.getAttribute().getKeyList();
        RolapSchema.PhysColumn nameCol = curLevel.getAttribute().getNameExp();
        RolapSchema.PhysColumn keyCol = keyList.get(keyList.size() - 1);
        if (nameCol != null && !nameCol.toString().equals(keyCol.toString())) {
            return false;
        }

        XmlaRequestContext context = XmlaRequestContext.getContext();
        // keep same as origin mondrian
        if (context == null) {
            return false;
        }
        ...
        if (context.clientType == XmlaRequestContext.ClientType.SMARTBI && !context.skipValidateMember) {
            return false;
        }
