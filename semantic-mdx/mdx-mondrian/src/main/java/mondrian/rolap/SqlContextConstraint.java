    ...
    implements MemberChildrenConstraint, TupleConstraint
{
        ...
        List<RolapMeasureGroup> measureGroupList)
    {
        ...
        if (cube.isVirtual()) {
            ...
            if (!findVirtualCubeStars(query, baseCubes, measureGroupSet, levels)) {
                return false;
            }
            assert levels != null;
        ...
        // May be duplicate with SqlConstraintUtils#containsCalculatedMember(Member[])
        Member[] members = context.getMembers();
        for (int i = 1; i < members.length; i++) {
            if (members[i].isCalculated()
                    && !((members[i].getExpression() instanceof ResolvedFunCall)
                    && ((ResolvedFunCall) members[i].getExpression()).getFunName().equals("Aggregate"))) {
                return false;
            }
        }
        return true;
            ...
            List<RolapCubeLevel> levels)
    {
        ...
        // originates from.
        addMeasures(query.getMeasuresMembers(), baseCubes, measureGroupSet);
        addMeasures(query.getNamedSetExpMembers(), baseCubes, measureGroupSet);
        if (measureGroupSet.size() == 0) {
            measureGroupSet.addAll(getMeasureGroupsBySourceType(levels));
        }
        return true;
    }

    private static void addMeasures(Set<Member> measureMembers,
                                    Collection<RolapCube> baseCubes,
                                    Set<RolapMeasureGroup> measureGroupSet) {
        for (Member member : measureMembers) {
    ...
    private static List<RolapMeasureGroup> getMeasureGroupsBySourceType(List<RolapCubeLevel> levels) {
        if (levels == null || levels.isEmpty()) {
            return Collections.emptyList();
        }
        String pureAxesCalculateSourceType = MondrianProperties.instance().PureAxesCalculateSourceType.get();
        int maxSnapshotCardinality = MondrianProperties.instance().SingleDimensionTableQueryCardinalityGap.get();

        int sourceDimensionCount = levelsSourceDimensionCount(levels);
        boolean useSnapshot = sourceDimensionCount == 1
                && (maxSnapshotCardinality < 0
                || maxSnapshotCardinality > 0
                && RolapUtil.getMaxCardinality(levels) < maxSnapshotCardinality);

        if (useSnapshot || pureAxesCalculateSourceType.equalsIgnoreCase(AXES_SOURCE_TYPE_SNAPSHOT)) {
            return Collections.emptyList();
        }
        if (pureAxesCalculateSourceType.equalsIgnoreCase(AXES_SOURCE_TYPE_PARTIAL_CUBE)) {
            return findPartialMeasureGroups(levels);
        }
        if (pureAxesCalculateSourceType.equalsIgnoreCase(AXES_SOURCE_TYPE_ALL_CUBE)) {
            return findAllMeasureGroups(levels);
        }
        return findPartialMeasureGroups(levels);
    }

    private static int levelsSourceDimensionCount(List<RolapCubeLevel> levels) {
        Set<RolapCubeDimension> dimensions = new HashSet<>();
    ...
    private static List<RolapMeasureGroup> findAllMeasureGroups(List<RolapCubeLevel> levels) {
        return levels.get(0).getCube().getMeasureGroups();
    }

    private static List<RolapMeasureGroup> findPartialMeasureGroups(List<RolapCubeLevel> levels) {
        List<RolapMeasureGroup> measureGroupList = new ArrayList<>();
        for (RolapCubeLevel level : levels) {
            if (level.isAll()) {
                continue;
            }
            RolapCubeDimension dimension = level.getDimension();
            if (!existInCurrentMG(dimension, measureGroupList)) {
                measureGroupList.add(findSuitableMG(dimension, dimension.getCube().getMeasureGroups()));
            }
        }
        return measureGroupList;
    }

    private static RolapMeasureGroup findSuitableMG(RolapCubeDimension dimension, List<RolapMeasureGroup> measureGroups) {
        for (RolapMeasureGroup measureGroup : measureGroups) {
            if (measureGroup.dimensionMap3.containsKey(dimension)) {
                return measureGroup;
            }
        }
        return null;
    ...
    private static boolean existInCurrentMG(RolapCubeDimension dimension, List<RolapMeasureGroup> measureGroupList) {
