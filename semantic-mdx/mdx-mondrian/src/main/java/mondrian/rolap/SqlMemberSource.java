    ...
    implements MemberReader, SqlTupleReader.MemberBuilder
{
    ...
    String makeChildMemberSql(
            ...
            SqlTupleReader.ColumnLayoutBuilder layoutBuilder, boolean fromDimTable)
    {
        Util.deprecated(
        ...
        final RolapStarSet starSet = new RolapStarSet(null, null, aggMeasureGroup);
        starSet.setCubeDimension(member.getDimension());
        final Dialect dialect = getDialect();
        final SqlQueryBuilder queryBuilder =
            new SqlQueryBuilder(
                dialect,
...
*/
        ...
        return projectProperties(
            ...
            level.attribute.getProperties(), fn, fromDimTable);
    }
            ...
            boolean fromDimTable)
    {
        ...
        if (level.attribute.getValueExp() != null) {
            final RolapSchema.PhysColumn exp = fn.apply(level.attribute.getValueExp());
            levelLayout.valueOrdinal = queryBuilder.addColumn(
                    queryBuilder.column(exp, dimension),
                    Clause.SELECT_GROUP, joiner, null);
        }
        ...
        Pair<String, List<SqlStatement.Type>> pair = null;
        if (!fromDimTable && queryBuilder.sqlQuery instanceof CustomizeSqlQuery) {
            for (RolapMeasureGroup measureGroup : level.getCube().getMeasureGroups()) {
                if (measureGroup.dimensionMap3.containsKey(level.getDimension())) {
                    Pair<RolapStar.Table, Map<String, RolapSchema.PhysPath>> factAndPath
                            = measureGroup.getStar().getFactAndPath();
                    pair = queryBuilder.toSqlAndTypes(factAndPath, null, null);
                    break;
                }
            }
        } else {
            pair = queryBuilder.toSqlAndTypes();
        }
...
//        levelBitKey.set(bitPosition);
        ...
        MemberChildrenConstraint constraint)
    {
        ...
        boolean fromDimTable = MondrianProperties.instance().FetchDimDataFromSnapshot.get();
        getMemberChildren2(parentMember, children, constraint, fromDimTable);
    }
        ...
        boolean fromDimTable)
    {
        ...
        SqlTupleReader.ColumnLayoutBuilder layoutBuilder =
            new SqlTupleReader.ColumnLayoutBuilder();
        if (parentLevel.isParentChild()) {
            sql = makeChildMemberSqlPC(parentMember, layoutBuilder, fromDimTable);
            parentChild = true;
            ...
            if (parentChild) {
                sql = makeChildMemberSql_PCRoot(parentMember, layoutBuilder, fromDimTable);
            } else {
                sql = makeChildMemberSql(
                    parentMember, constraint, layoutBuilder, fromDimTable);
            }
        }
        if (sql == null) {
            return;
        ...
        if (!sql.contains("limit") &&
                ...
                (sql.contains("LIKE '%") && MondrianProperties.instance().FilterRowLimit.get() > 0))) {
        ...
        try {
            ...
            // if we can not get data from dimension snapshot, we can get from cube
            if (fromDimTable) {
                if (stmt != null) {
                    stmt.close();
                }
                getMemberChildren2(parentMember, children, constraint, false);
                return;
            } else {
                throw e;
            }
                ...
                if (member == null) {
                    ...
                    member =
                        makeMember(
                            ...
                            nameValue, orderKey, value, parentChild, stmt, layout);
                }
                children.add(member);
            }
        } catch (SQLException e) {
        ...
        throws SQLException
    {
        ...
        if (key instanceof List && (((List) key).size() >= 1)) {
            memberKey = ((List) key).get(((List) key).size() - 1).toString();
        } else {
            memberKey = key.toString();
        }
        RolapMemberBase member =
                new RolapMemberBase(
                        ...
                        MemberType.REGULAR,
                        RolapMemberBase.deriveUniqueName(
                                parentMember, childLevel, memberKey, false),
                        builder.build());
        ...
        boolean fromDimTable)
    {
        ...
        return projectProperties(
            ...
            Util.<RolapSchema.PhysColumn>identityFunctor(), fromDimTable);
    }
