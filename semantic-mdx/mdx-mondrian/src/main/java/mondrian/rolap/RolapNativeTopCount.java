...
public class RolapNativeTopCount extends RolapNativeSet {
    ...
    static class TopCountConstraint extends SetConstraint {
        ...
        {
            queryBuilder.sqlQuery.setOrderAscending(!ascending);
            if (orderByExpr != null) {
                ...
                Util.getMeasureGroupList(measureGroupList, orderByExpr);
                if (orderByExpr instanceof MemberExpr
                        && ((MemberExpr)orderByExpr).getMember() instanceof RolapHierarchy.RolapCalculatedMeasure) {
                    if (!measureGroupList.isEmpty()) {
                        addSelectAndOrderBy(sqlQuery, orderBySql, nullable);
                    }
                } else if (orderByExpr.getType().getDimension() != null) {
                    addSelectAndOrderBy(sqlQuery, orderBySql, nullable);
                }
            }
            if (topCount != null && topCount > 0) {
                queryBuilder.sqlQuery.setHasLimit(true);
                queryBuilder.sqlQuery.setLimitCount(topCount);
            }
            super.addConstraint(queryBuilder, starSet);
        }

        private void addSelectAndOrderBy(SqlQuery sqlQuery, String orderBySql, boolean nullable) {
        ...
        Exp[] args)
    {
        ...
        if (args.length == 3) {
            ...
            Util.extractAllBaseMembers(orderByExpr, members);

            if (members.size() > 1) {
                return null;
            }

            boolean measureOnFactTable = members.stream().allMatch(member -> {
                if (!(member instanceof RolapStoredMeasure)) {
                    return false;
                }
                RolapStar.Measure starMeasure = ((RolapStoredMeasure)member).getStarMeasure();
                RolapStar.Table factTable = starMeasure.getStar().getFactTable();
                return factTable.equals(starMeasure.getTable());
            });
            if (!measureOnFactTable) {
                return null;
            }
            ...
            Util.extractMeasureGroupsFromMeasures(measureGroupList, members);
        }
