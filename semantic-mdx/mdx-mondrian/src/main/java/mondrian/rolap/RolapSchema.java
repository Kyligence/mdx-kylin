...
public class RolapSchema extends OlapElementBase implements Schema {
    ...
    public static class RolapSchemaFunctionTable extends FunTableImpl {
        ...
        public RolapSchemaFunctionTable(Collection<UdfResolver.UdfFactory> udfs) {
    ...
    public static class PhysTable extends PhysRelationImpl
    {
        ...
        @Override
        public String toString() {
            return (schemaName == null ? "" : (physSchema.dialect.quoteIdentifier(schemaName) + '.'))
                + physSchema.dialect.quoteIdentifier(name)
                + (name.equals(alias) ? "" : (" as " + physSchema.dialect.quoteIdentifier(alias)));
        }
                ...
                if (column == null) {
                    column =
                        new PhysRealColumn(
                            ...
                            jdbcColumn.getSqlType(physSchema.dialect),
        ...
        public Map<String, PhysColumn> getColumnsMap() {
            return Collections.unmodifiableMap(columnsByName);
        }
    ...
    public static class PhysLink implements DirectedGraph.Edge<PhysRelation> {
        ...
        public List<PhysColumn> getFromColumnList() {
            return columnList;
        }

        public List<PhysColumn> getToColumnList() {
            return sourceKey.getColumnList();
        }
            ...
            String table)
        {
            ...
            if (tableMap.containsKey(key)) {
                ...
                for (StatisticsProvider statisticsProvider
                    : statisticsProviders)
                {
                    if (XmlaRequestContext.getContext() != null) {
