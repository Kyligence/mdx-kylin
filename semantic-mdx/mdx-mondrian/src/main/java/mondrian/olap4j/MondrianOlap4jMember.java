    ...
    implements Member, Named
{
    ...
    public int getChildMemberCount() throws OlapException {
        if (!MondrianProperties.instance().SupportDisplayInfoMemberChildrenCount.get()) {
            return -1;
        }

        if (member instanceof RolapMember) {
            if (member.getLevel().getChildLevel() == null) {
                return 0;
            }

            HierarchyCache hierarchyCache = CacheManager.getCacheManager().getHierarchyCache();
            if (hierarchyCache.isCacheEnabled()) {
                HierarchyMemberTree hierarchyMemberTree = hierarchyCache.getMemberTreeIfPresent(
                        (RolapCubeHierarchy)member.getHierarchy());
                if (hierarchyMemberTree != null) {
                    return hierarchyMemberTree.getChildrenMembersCount(member.getUniqueName());
                }
            }

            if (member.isAll()) {
                OptionalLong cardinalityFromMetaData = RolapUtil.getCardinalityFromMetaData(
                        ((RolapMember)member).getLevel().getChildLevel());
                if (cardinalityFromMetaData.isPresent()) {
                    return (int)cardinalityFromMetaData.getAsLong();
                }
            }
        }
    ...
    public MondrianOlap4jMember getParentMember() {
        ...
        if (parentMember == null || !olap4jSchema.olap4jCatalog.olap4jDatabaseMetaData
