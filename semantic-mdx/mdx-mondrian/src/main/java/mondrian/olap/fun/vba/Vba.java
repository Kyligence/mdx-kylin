...
public class Vba {
    ...
    @FunctionName("CDate")
    @Signature("CDate(date/int)")
    @Description(
        "Returns an expression that has been converted to a Variant of subtype "
        + "Date. if the type of argument is int, convert the offset of days to specific date.")
    public static Date cDate(Object expression) {
        ...
        if (expression instanceof Date) {
            ...
            try {
                ...
                Date date = dateFormat.parse("1899-12-30");
                Integer dateOffset = Integer.valueOf(str);
                date.setTime(date.getTime() + (long) dateOffset * MILLIS_IN_A_DAY);
                return date;
        ...
        } else if (expression == null || str.contentEquals(MondrianProperties.instance().NullMemberRepresentation.get())) {
            return null;
        } else {
            // note that this currently only supports a limited set of dates and times
            // "October 19, 1962"
            // "4:35:47 PM"
            // java.sql.Date only supports yyyy-MM-dd which is consistent with Date of DEFAULT CHINA
            try {
                return _cDate(str, Locale.getDefault());
            } catch (InvalidArgumentException e) {
                return _cDate(str, Locale.CHINA);
            }
        }
    }

    private static Date _cDate(String str, Locale locale) {
        try {
            return DateFormat.getTimeInstance(DateFormat.DEFAULT, locale).parse(str);
        } catch (ParseException ignored) {
        }
        try {
            return DateFormat.getDateTimeInstance(DateFormat.DEFAULT, DateFormat.DEFAULT, locale).parse(str);
        } catch (ParseException ignored) {
        }
        try {
            return DateFormat.getDateInstance(DateFormat.DEFAULT, locale).parse(str);
        } catch (ParseException ignored) {
        }
        throw new InvalidArgumentException(
                "Invalid parameter. "
     ...
     * DateDiff function
     */
    private static int computeDiffInDays(Calendar cal1, Calendar cal2) {
        boolean inverse;
    ...
    @FunctionName("InStr")
    @Signature("InStr(stringcheck, stringmatch[, compare])")
    @Description(
            "Returns the position of an occurrence of one string within "
                    + "another.")
    public static int inStr(
    ...
    {
        return inStr(1, stringCheck, stringMatch, compare);
    }
...
//    }
    ...
    private enum Interval {
        ...
        int diff(Calendar calendar1, Calendar calendar2, int firstDayOfWeek) {
            switch (this) {
                case yyyy:
                    return floor(calendar2).get(dateField) - floor(calendar1).get(dateField);
                case q:
                    return computeDiffInMonths(calendar1, calendar2) / 3;
                case m:
                    return computeDiffInMonths(calendar1, calendar2);
                case w:
                    return computeDiffInDays(calendar1, calendar2) / 7;
                case ww:
                    calendar1.setFirstDayOfWeek(firstDayOfWeek);
                    calendar2.setFirstDayOfWeek(firstDayOfWeek);
                    return computeDiffInDays(floor(calendar1), floor(calendar2)) / 7;
                case y:
                case d:
                    // don't use d's dateField (DAY_OF_MONTH)
                    return computeDiffInDays(calendar1, calendar2);
                case h:
                case n:
                case s:
                    long diffSeconds = floor(calendar2).getTimeInMillis() / 1000
                            - floor(calendar1).getTimeInMillis() / 1000;
                    if (this == h) {
        ...
        private int computeDiffInMonths(Calendar calendar1, Calendar calendar2) {
            calendar1 = floor(calendar1);
            calendar2 = floor(calendar2);
            int monthsOfYear = 12 * (calendar2.get(Calendar.YEAR) - calendar1.get(Calendar.YEAR));
            int monthsInYear = calendar2.get(Calendar.MONTH) - calendar1.get(Calendar.MONTH);
            return monthsOfYear + monthsInYear;
        }
