...
public class AbstractAggregateFunDef extends FunDefBase {
        ...
        Validator validator, Exp[] args, int i, int category)
    {
        ...
        if (i == 0) {
            ...
            && (Objects.isNull(XmlaRequestContext.getContext()) || !XmlaRequestContext.getContext().doValidateCM)) {
                Exp arg = args[0];
                //following part will add extra Cache() function in pattern and statement, may cause unmatch
    ...
    public static TupleList processUnrelatedDimensions(
        ...
        Evaluator evaluator)
    {
        if (MondrianProperties.instance().IgnoreMondrianProcessUnrelatedDimensions.get()) {
