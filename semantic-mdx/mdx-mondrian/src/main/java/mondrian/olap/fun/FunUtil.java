...
public class FunUtil extends Util {
    ...
    public static final NullLevel NullLevel = new NullLevel();
    ...
    /**
     * Extracts the common level from several members that are merged in a calculated member
     * with the "Aggregate" function. This is usually used by the Excel filter.
     * @param member The merged calculated member.
     * @return The level of the merged members if they belong to the same level, or the level
     *         of the calculated member if they do not.
     */
    static Level evaluateAggregateMemberLevel(RolapCalculatedMember member) {
        Exp aggregate = member.getExpression();
        if (!(aggregate instanceof ResolvedFunCall && ((ResolvedFunCall)aggregate).getFunDef() instanceof AggregateFunDef)) {
            return null;
        }

        Exp aggregateSet = ((ResolvedFunCall)aggregate).getArg(0);
        if (!(aggregateSet instanceof ResolvedFunCall && ((ResolvedFunCall)aggregateSet).getFunDef() instanceof SetFunDef)) {
            return null;
        }

        Level level = null;
        for (Exp aggregateMember : ((ResolvedFunCall)aggregateSet).getArgs()) {
            if (!(aggregateMember instanceof MemberExpr)) {
                return member.getLevel();
            }

            if (level == null) {
                level = ((MemberExpr)aggregateMember).getMember().getLevel();
            } else if (!level.equals(((MemberExpr)aggregateMember).getMember().getLevel())) {
                return null;
            }
        }
            ...
            if (slicerMembers.size() > 0) {
                for (RolapMember slicerMember : slicerMembers) {
                    ...
                    for (Member member : evaluator.getMembers()) {
                        if (slicerMember.equals(member)) {
                            break;
                        }
                        slicerIdx++;
                    }
                    if (slicerIdx >= 0 && slicerIdx < evaluator.getMembers().length) {
...
//            }
    ...
    public static TupleList hierarchizeTupleList(
        ...
        boolean post)
    {
        return hierarchizeTupleListWithFixedIndex(tupleList, post, -1, null);
    }

    /**
     * Sorts a list of tuples with the defined position in fixed order
     * and other members in hierarchical order.
     * @param tupleList List of tuples.
     * @param post Whether to sort in post order; if false, sorts in pre order.
     * @param fixedOrderIndex The position to be sorted in fixed order.
     * @param fixedMemberIndices The map which indicates the fixed order.
     */
    public static TupleList hierarchizeTupleListWithFixedIndex(
            TupleList tupleList,
            boolean post,
            int fixedOrderIndex,
            Map<? extends Member, Integer> fixedMemberIndices)
    {
        ...
        if (tupleList.getArity() == 1 && fixedOrderIndex != 0) {
            hierarchizeMemberList(fixedList.slice(0), post);
            return fixedList;
        }

        Comparator<List<Member>> comparator;
        if (fixedOrderIndex < 0) {
            comparator = new HierarchizeTupleComparator(fixedList.getArity(), post).wrap();
        } else {
            comparator = new HierarchizeTupleFixedIndexComparator(fixedList.getArity(), post, fixedOrderIndex, fixedMemberIndices).wrap();
        }
    ...
    static List<Member> periodsToDate(
        ...
        RolapCubeHierarchy hierarchy = (RolapCubeHierarchy) level.getHierarchy();
        if (member == null) {
            member = evaluator.getContext(hierarchy);
        }
        HierarchyCache hierarchyCache = CacheManager.getCacheManager().getHierarchyCache();
        HierarchyMemberTree hierarchyMemberTree = null;
        if (hierarchyCache.isCacheEnabled()) {
            try {
                hierarchyMemberTree = hierarchyCache.getMemberTree(hierarchy);
                int depthsUp = member.getLevel().getDepth() - level.getDepth();
                if (depthsUp < 0) {
                    return Collections.emptyList();
                }
                RolapMember levelParentMember = (RolapMember) member;
                for (int i = depthsUp; i > 0; i--) {
                    levelParentMember = levelParentMember.getParentMember();
                }
                RolapMember firstDescendantLevelMember = hierarchyMemberTree.getFirstDescendantLevelMember(levelParentMember.getUniqueName(), depthsUp);

                List xtdMembers = hierarchyMemberTree.getMembersBetween(firstDescendantLevelMember.getUniqueName(), true, member.getUniqueName(), true);
                if (xtdMembers != null) {
                    return xtdMembers;
                }
        ...
        Member member2)
    {
        ...
        // for time columns, choose member using orderkey, not ordinal
        if (member1.getLevel() != null && (
                member1.getLevel().getLevelType() == org.olap4j.metadata.Level.Type.TIME_YEARS
                        || member1.getLevel().getLevelType() == org.olap4j.metadata.Level.Type.TIME_QUARTERS
                        || member1.getLevel().getLevelType() == org.olap4j.metadata.Level.Type.TIME_MONTHS
                        || member1.getLevel().getLevelType() == org.olap4j.metadata.Level.Type.TIME_WEEKS
                        || member1.getLevel().getLevelType() == org.olap4j.metadata.Level.Type.TIME_DAYS
        )) {
            for (Member m : cousins) {
                if (m.getOrderKey() == member1.getOrderKey()) {
                    mReturn = m;
                    break;
                }
            }
    ...
    static List<Member> getDescendantsFromHierarchyCache(Member member,
                                                         int startDepth,
                                                         int endDepth) {
        ...
        RolapCubeHierarchy hierarchy = (RolapCubeHierarchy)member.getHierarchy();
        ...
        if (hierarchyCache.isCacheEnabled()) {
            try {
                HierarchyMemberTree hierarchyMemberTree = hierarchyCache.getMemberTree(hierarchy);
                List<? extends Member> result = hierarchyMemberTree.getDescendantMembers(member.getUniqueName(), startDepth, endDepth);
...
//        final Comparable k2 = m2.getOrderKey();
        ...
        Comparable k1;
        Comparable k2;
        if (m1 instanceof VisualTotalsFunDef.VisualTotalMember) {
            k1 = ((VisualTotalsFunDef.VisualTotalMember) m1).getMember().getOrderKey();
        } else {
            k1 = m1.getOrderKey();
        }
        if (m2 instanceof VisualTotalsFunDef.VisualTotalMember) {
            k2 = ((VisualTotalsFunDef.VisualTotalMember) m2).getMember().getOrderKey();
        } else {
            k2 = m2.getOrderKey();
        }
  ...
  /**
        ...
        final boolean includeCalcMembers)
    {
        ...
        // MDX-Service #85 单个维度过滤显示错误
        XmlaRequestContext context = XmlaRequestContext.getContext();
        boolean isFromSmartBI = context != null && context.clientType.equals(XmlaRequestContext.ClientType.SMARTBI);

        if (evaluator.isNonEmpty() || isFromSmartBI) {
    ...
    */
        ...
        Hierarchy hierarchy)
    {
        ...
        if (builder.memberList.size() > 0) {
            members[0] = builder.memberList.get(0);
        }
        return i;
    }
    ...
    public static List<Member> getChildrenOfMember(Member parent) {
        HierarchyCache hierarchyCache = CacheManager.getCacheManager().getHierarchyCache();
        assert parent instanceof RolapMember;
        List<RolapMember> children;
        try {
            HierarchyMemberTree memberTree = hierarchyCache.getMemberTree(((RolapMember) parent).getHierarchy());
            children = memberTree.getChildrenMembers(parent.getUniqueName());
        } catch (ExecutionException | SQLException e) {
            throw new MondrianException(e);
        }
    ...
    public static List<Member> getHierarchyAllMembers(Hierarchy hierarchy) {
        assert hierarchy instanceof RolapCubeHierarchy;
        HierarchyCache hierarchyCache = CacheManager.getCacheManager().getHierarchyCache();
        assert hierarchy instanceof RolapCubeHierarchy;
        List<RolapMember> allMembers = new ArrayList<>();
        try {
            HierarchyMemberTree memberTree = hierarchyCache.getMemberTree((RolapCubeHierarchy) hierarchy);
            allMembers = memberTree.getAllMembers();
        } catch (ExecutionException | SQLException e) {
            throw new MondrianException(e);
        }
     ...
     */
    public static abstract class TupleComparator
        implements Comparator<List<Member>>
    {
        ...
        protected TupleComparator(int arity) {
            this.arity = arity;
        }
     ...
     */
    public static class HierarchizeTupleComparator extends TupleComparator {
        final boolean post;

        public HierarchizeTupleComparator(int arity, boolean post) {
            super(arity);
            this.post = post;
        }
    ...
    private static class HierarchizeTupleFixedIndexComparator extends HierarchizeTupleComparator {
        private final int excludingIndex;
        private final Map<? extends Member, Integer> fixedMemberIndices;

        HierarchizeTupleFixedIndexComparator(int arity, boolean post, int fixedIndex, Map<? extends Member, Integer> fixedMemberIndices) {
            super(arity, post);
            this.excludingIndex = fixedIndex;
            this.fixedMemberIndices = fixedMemberIndices;
        }
        ...
        public int compare(List<Member> a1, List<Member> a2) {
            for (int i = 0; i < arity; i++) {
                ...
                int c;
                if (i == excludingIndex) {
                    c = Objects.compare(fixedMemberIndices.get(m1), fixedMemberIndices.get(m2), Comparator.naturalOrder());
                } else {
                    c = FunUtil.compareHierarchically(m1, m2, post);
                }
                if (c != 0) {
                    return c;
                }
...
*/
    ...
    public static class NullLevel implements Level {
