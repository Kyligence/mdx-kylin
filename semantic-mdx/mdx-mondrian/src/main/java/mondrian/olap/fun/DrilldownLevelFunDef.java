            ...
            return new AbstractListCalc(call, new Calc[] {listCalc, indexCalc})
            {
                public TupleList evaluateList(Evaluator evaluator) {
                    ...
                    for (List<Member> tuple : list) {
                        ...
                        assert tuple.get(index).getHierarchy() instanceof RolapCubeHierarchy;
                        List<Member> children = new ArrayList<>();
                        if (hierarchyCache.isCacheEnabled()) {
                            children.addAll(FunUtil.getChildrenOfMember(tuple.get(index)));
                        } else {
                            children = schemaReader.getMemberChildren(tuple.get(index));
                        }
    ...
    List<Member> drill(int searchDepth, List<Member> list, Evaluator evaluator)
    {
        ...
        for (int i = 0, m = list.size(); i < m; i++) {
                ...
                && !FunUtil.isAncestorOf(member, nextMember, true))
            {
                ...
                HierarchyCache hierarchyCache = CacheManager.getCacheManager().getHierarchyCache();
                if (hierarchyCache.isCacheEnabled()) {
                    assert member.getHierarchy() instanceof RolapCubeHierarchy;
                    try {
                        HierarchyMemberTree memberTree = hierarchyCache.getMemberTree((RolapCubeHierarchy) member.getHierarchy());
                        childMembers.addAll(memberTree.getChildrenMembers(member.getUniqueName()));
                    } catch (ExecutionException | SQLException e) {
                        throw new MondrianException(e);
                    }
                } else {
                    childMembers = evaluator.getSchemaReader().getMemberChildren(member, evaluator);
                }
