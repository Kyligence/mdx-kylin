...
public abstract class FunTableImpl implements FunTable {
    ...
    public final void init() {
        ...
        final String[] reservedWords = builder.reservedWords.toArray(new String[0]);
    ...
    private static class BuilderImpl implements Builder {
        ...
        private final Map<Pair<String, Syntax>, List<Resolver>> mapNameToResolvers = new HashMap<>();
        ...
        public void define(Resolver resolver) {
            ...
            for (String reservedWord : resolver.getReservedWords()) {
        ...
        protected void organizeFunctions() {
            ...
            final List<List<Resolver>> nonSingletonResolverLists = new ArrayList<>();
            for (Resolver resolver : resolverList) {
                Pair<String, Syntax> key = makeResolverKey(
                        resolver.getName(),
                        resolver.getSyntax());
            ...
            final Comparator<Resolver> comparator = Comparator.comparing(Resolver::getSignature);
            for (List<Resolver> resolverList : nonSingletonResolverLists) {
                resolverList.sort(comparator);
            }
        }
    }
}
