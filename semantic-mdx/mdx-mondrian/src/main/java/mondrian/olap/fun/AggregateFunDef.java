...
public class AggregateFunDef extends AbstractAggregateFunDef {
    ...
    public static class AggregateCalc extends GenericCalc {
            ...
            TupleList tupleList)
        {
            ...
            boolean enableFilterPushdown = MondrianProperties.instance().EnableFilterPushdown.get()
                    && tupleList.size() <= MondrianProperties.instance().FilterPushdownInClauseMaxSize.get();
            boolean nonAccumulative = (aggregator == RolapAggregator.DistinctCount) || (aggregator ==RolapAggregator.Avg)
                    || ((aggregator == RolapAggregator.Sum || aggregator == RolapAggregator.Count) && evaluator.getMeasureGroup().isM2M());

            if (!nonAccumulative && !enableFilterPushdown)
            {
                Member measureMember = evaluator.getNonAllMembers()[0];
                boolean noRelation = false;
                if (measureMember instanceof RolapBaseCubeMeasure) {
                    RolapMeasureGroup measureGroup = ((RolapBaseCubeMeasure) measureMember).getMeasureGroup();

                    // Check if the current base measure and tuple members are from different cubes,
                    // i.e. have no relation.
                    noRelation = true;
                    List<Member> aggregateTuple0 = tupleList.get(0);
                    for (Member member : aggregateTuple0) {
                        if (!(member instanceof RolapMember)
                                || measureGroup.getPath(((RolapMember) member).getDimension()) != null) {
                            noRelation = false;
                            break;
                        }
                    }
                ...
                try {
                    ...
                    // If there is no relation between the current base measure and tuple members, we should not
                    // do aggregation here, since it may result in a wrong accumulation.
                    if (noRelation) {
                        return calc.evaluate(evaluator);
                    }

                    boolean tableFlag = XmlaRequestContext.getContext().tableauFlag;
                    if (tableFlag) {
                        tupleList = removeRepeatTuple(tupleList);
                    }
                    final Object o =
                        rollup.aggregate(
        ...
        private static TupleList removeRepeatTuple(TupleList tupleList) {
            // Obtain the depth of lowest level
            int depth = 0;
            int tupleLength = tupleList.size();
            for (int i = 0; i < tupleLength; i++) {
                String tmp = tupleList.get(i).get(0).getUniqueName();
                if (tmp == null) {
                    continue;
                }
            ...
            // Obtain the member which isn't repeated
            ...
            // Remove repeated member
            TupleList noRepeatTupleList = new UnaryTupleList();
            ...
            for (int i = 0; i < tupleLength; i++) {
                ...
                boolean noRepeateFlag = true;
                for (int k = 0; k < parentListLen; k++) {
                    if (!tmp.equals(parentList.get(k)) && tmp.startsWith(parentList.get(k))) {
                        noRepeateFlag = false;
                        break;
                    }
                }
                if (noRepeateFlag) {
                    RolapMemberBase tmpRolapMemberBase = (RolapMemberBase) tupleList.get(i).get(0);
                    List<Member> memberList = Collections.singletonList(tmpRolapMemberBase);
                    noRepeatTupleList.add(memberList);
                }
            }
            return noRepeatTupleList;
        }
            ...
            TupleList tupleList)
        {
            ...
            if (!MondrianProperties.instance().EnableOptimizeTupleSizeInAggregate.get()) {
                return false;
            }
        ...
        private static void checkIfAggregationSizeIsTooLarge(List list) {
            final IntegerProperty property =
                MondrianProperties.instance().FilterPushdownInClauseMaxSize;
