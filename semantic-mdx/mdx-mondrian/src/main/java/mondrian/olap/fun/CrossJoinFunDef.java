    ...
    public Type getResultType(Validator validator, Exp[] args) {
        ...
        for (Exp arg : args) {
            ...
            if (type instanceof SetType) {
                ...
                throw validator.getPrintStackEx(Util.newInternal("arg to crossjoin must be a set"));
            }
        }
        final MemberType[] types = list.toArray(new MemberType[list.size()]);
        TupleType.checkHierarchies(types, validator);
                ...
                public TupleCursor tupleCursor() {
                    return new AbstractTupleCursor(getArity()) {
                        ...
                        public boolean forward() {
                            Util.checkCJResultLimit(currentCount, resultLimit);
                            if (i2.forward()) {
                                currentCount++;
                                return true;
                            }
