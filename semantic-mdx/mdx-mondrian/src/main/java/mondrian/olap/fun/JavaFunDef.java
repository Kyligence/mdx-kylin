...
public class JavaFunDef extends FunDefBase {
    ...
    private static class JavaMethodCalc extends GenericCalc {
        ...
        private final BitKey stringParameters;
            ...
            ResolvedFunCall call, Calc[] calcs, Method method)
        {
            ...
            List<Integer> sizeAndPositions = new ArrayList<>(parameterTypes.length + 1);
            sizeAndPositions.add(parameterTypes.length);
            for (int i = 0; i < parameterTypes.length; i++) {
                Class<?> parameterType = parameterTypes[i];
                if (parameterType == String.class) {
                    sizeAndPositions.add(i);
                }
            }
            this.stringParameters = BitKey.Factory.createBitKeyWithSizeAndPositions(sizeAndPositions);
        }
        ...
        public Object evaluate(Evaluator evaluator) {
            ...
            for (int i = 0; i < args.length; i++) {
                ...
                if (stringParameters.get(i) && !(args[i] instanceof String)) {
