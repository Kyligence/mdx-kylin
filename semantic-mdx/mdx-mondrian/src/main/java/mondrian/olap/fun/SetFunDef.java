...
public class SetFunDef extends FunDefBase {
    ...
    public SetFunDef withNewArgTypes(int[] newArgTypes) {
        return new SetFunDef(Resolver, newArgTypes);
    }
            ...
            for (int i = 0; i < args.length; i++) {
                ...
                if (i == 0) {
                    ...
                    if (!TypeUtil.isUnionCompatible(type0, type)) {
                        throw validator.getPrintStackEx(MondrianResource.instance()
                ...
                return new AbstractVoidCalc(arg, new Calc[]{memberCalc}) {
                    ...
                    public void evaluateVoid(Evaluator evaluator) {
                        ...
                        if (member instanceof VisualTotalsFunDef.VisualTotalMember) {
                            ((VisualTotalsFunDef.VisualTotalMember) member).getChildMemberList().forEach(
                                    actualMember -> result.addTuple(actualMember));
                        } else {
