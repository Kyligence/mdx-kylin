...
public class DescendantsFunDef extends FunDefBase {
        ...
        if ((depthSpecified || depthEmpty) && flag.leaves) {
                ...
                call, new Calc[] {memberCalc, levelCalc})
            {
                public TupleList evaluateList(Evaluator evaluator) {
                    ...
                    List<Member> result = new ArrayList<>();
                        descendantsByLevel(
                            evaluator.getSchemaReader(), member, level, result,
    ...
    private static void descendantsByDepth(
        ...
        Evaluator context)
    {
        SchemaReader finalSchemaReader = schemaReader;
        final boolean useHierarchyCache = MondrianProperties.instance().MSTRHierarchyCacheOptimizationEnabled.get();
        if (useHierarchyCache && member.getHierarchy().getLevelList().size() > 2) {
            finalSchemaReader = new CacheDelegatingSchemaReader(schemaReader, member.getHierarchy());
        }
        ...
        for (int depth = 0;; ++depth) {
            ...
            children = finalSchemaReader.getMemberChildren(children, context);
            if (children.size() == 0) {
                break;
            }
        }
        ...
        final int depthLimit)
    {
        ...
        if (!finalSchemaReader.isDrillable(member)) {
            if (depthLimit >= 0) {
                result.add(member);
            }
            return;
        ...
        for (int depth = 0; depthLimit == -1 || depth <= depthLimit; ++depth) {
            children = finalSchemaReader.getMemberChildren(children);
            if (children.size() == 0) {
            ...
            for (Member child : children) {
                ...
                if (finalSchemaReader.isDrillable(child)) {
        ...
        Evaluator context)
    {
        ...
        final boolean useHierarchyCache = MondrianProperties.instance().MSTRHierarchyCacheOptimizationEnabled.get();
        if (useHierarchyCache && ancestor.getHierarchy().getLevelList().size() > 2) {
            schemaReader = new CacheDelegatingSchemaReader(schemaReader, ancestor.getHierarchy());
        }
        ...
        if (leaves) {
            ...
            do {
                ...
                for (Member member : members) {
                    ...
                    if (childMembers.size() == 0 || currentDepth == levelDepth) {
                            result.add(member);
                    } else {
    ...
    static boolean checkDescendantsLeaves(Evaluator evaluator, CrossJoinArgFactory crossJoinArgFactory, Exp exp) {
        if (!(exp instanceof ResolvedFunCall))
            return false;
        ResolvedFunCall descendantsFunCall = (ResolvedFunCall) exp;

        CrossJoinArg[] checkedArgs = crossJoinArgFactory.checkDescendants(
                            evaluator,
                            evaluator.getSchemaReader().getRole(),
                            descendantsFunCall.getFunDef(),
                            descendantsFunCall.getArgs());

        if (checkedArgs == null || !(checkedArgs[0] instanceof DescendantsCrossJoinArg))
            return false;
        DescendantsCrossJoinArg descendantsArg = (DescendantsCrossJoinArg) checkedArgs[0];

        return descendantsArg.isLeaves();
    }
    ...
    enum Flag {
        ...
        public Pair<Integer, Integer> getDepthRange(int memberDepth, int targetDepth, int maxDepth) {
            assert memberDepth >= 0 && targetDepth >= memberDepth;
            if (leaves) {
                return Pair.of(maxDepth, maxDepth);
            }

            assert before || self || after;
            int startDepth = before ? memberDepth : self ? targetDepth : targetDepth + 1;
            int endDepth = after ? maxDepth : self ? targetDepth : targetDepth - 1;

            startDepth = Math.min(startDepth, endDepth);
            return Pair.of(startDepth, endDepth);
        }
    }
}
