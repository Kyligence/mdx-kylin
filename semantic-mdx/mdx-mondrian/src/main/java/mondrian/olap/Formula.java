...
public class Formula extends QueryPart {
    ...
    public boolean equals(Object o) {
        return o instanceof Formula
                && this.getExpression().toString().contentEquals(((Formula) o).getExpression().toString());
    }
    ...
    void accept(Validator validator) {
        ...
        if (isMember) {
            if (!TypeUtil.canEvaluate(type)) {
                throw validator.getPrintStackEx(MondrianResource.instance().MdxMemberExpIsSet.ex(
                    exp.toString()));
            }
        } else {
            if (!TypeUtil.isSet(type)) {
                throw validator.getPrintStackEx(MondrianResource.instance().MdxSetExpNotSet.ex(id));
            }
        }
    ...
    private Exp getFormatExp(Validator validator) {
        ...
        if (type instanceof DecimalType) {
            ...
            return Literal.createString(formatString.toString());
        }
