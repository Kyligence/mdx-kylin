...
public class Query extends QueryPart {
    ...
    /**
     * Cube this query belongs to, is null when sub-query.
     */
    @Getter
    @Setter
    private Cube cube;
    ...
    /**
     * Priority of use cube, just use when sub-query.
     */
    @Getter
    @Setter
     ...
     */
    private final Set<FunDef> alertedNonNativeFunDefs = new HashSet<>();
     ...
     * for virtual cubes.
     */
    private final Set<Member> measuresMembers = new HashSet<>();

    private final Set<Member> namedSetExpMembers = new HashSet<>();
    ...
    @Getter
    private final boolean skipResolve;

    /**
     * Creates a Query.
     * At lease provide cubeName or mdxCube when skipResolve is true.
     */
    public Query(
            Statement statement,
            String cubeName,
        ...
        if (!skipResolve || statement != null) {
            statement.setQuery(this);
        }
        if (!skipResolve && mdxCube == null && cubeName == null) {
            throw new RuntimeException("Please at lease set cubeName or mdxCube");
        }
        this.cubeName = cubeName;
        if (mdxCube != null) {
            this.cube = mdxCube;
        } else if (!skipResolve) {
            this.cube = Util.lookupCube(statement.getSchemaReader(), cubeName, true);
        }
        ...
        if (RolapUtil.PROFILE_LOGGER.isDebugEnabled()
                && statement != null && statement.getProfileHandler() == null) {
     ...
     * tree in any way.
     *
     * Notice: if skipResolve is true, skip it
     */
    public void resolve() {
        if (skipResolve) {
            return;
    ...
    public void resolve(Validator validator) {
        ...
        if (axes != null) {
            ...
            for (QueryAxis axis : axes) {
                ...
                if (!axisNames.add(axis.getAxisOrdinal().logicalOrdinal())) {
                    throw validator.getPrintStackEx(MondrianResource.instance().DuplicateAxis.ex(
                        axis.getAxisName()));
                }
            }
                ...
                if (!axisNames.contains(seekOrdinal)) {
                    AxisOrdinal axisName =
                        AxisOrdinal.StandardAxisOrdinal.forLogicalOrdinal(
                            seekOrdinal);
                    throw validator.getPrintStackEx(MondrianResource.instance().NonContiguousAxis.ex(
                        seekOrdinal,
                        axisName.name()));
                }
                ++seekOrdinal;
        ...
        for (Hierarchy hierarchy : ((RolapCube) getCube()).getHierarchyList()) {
            ...
            if (useCount > 1) {
                throw validator.getPrintStackEx(MondrianResource.instance().HierarchyInIndependentAxes.ex(
...
//        }
    ...
    public Object[] getChildren() {
        // Children are axes, slicer, and formulas (in that order, to be
    ...
    public void addMeasuresMembers(OlapElement olapElement)
    {
        ...
        if (olapElement instanceof NamedSet) {
            NamedSet namedSet = (NamedSet)olapElement;
            RolapUtil.findMembersFromExp(namedSet.getExp(), namedSetExpMembers);
        }
    }
    ...
    public Set<Member> getNamedSetExpMembers() {
        return Collections.unmodifiableSet(namedSetExpMembers);
    }
