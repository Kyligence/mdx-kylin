...
public class QueryAxis extends QueryPart {
    ...
    public void resolve(Validator validator) {
        ...
        if (!TypeUtil.isSet(type)) {
                ...
                || type instanceof HierarchyType)
            {
                ...
                throw validator.getPrintStackEx(MondrianResource.instance().MdxAxisIsNotSet.ex(
                    axisOrdinal.name()));
            }
        }
    }
    ...
    public String toStringInStack(ArrayStack<QueryPart> stack, int pos) {
        ...
        buf.append(" ").append("nonEmpty=").append(nonEmpty);
        buf.append(" ").append("ordered=").append(ordered);
        buf.append(" ").append("exp=");
        addExp(exp, buf, stack, pos);
        buf.append(" ").append("axisOrdinal=").append(axisOrdinal.name());
        buf.append(" ").append("subtotalVisibility=").append(subtotalVisibility.name());
        if (dimensionProperties != null && dimensionProperties.length > 0) {
            buf.append(" ").append("dimensionProperties=").append("[");
            for (int i = 0; i < dimensionProperties.length; i++) {
                if (i > 0) {
                    buf.append(", ");
                }
                Id id = dimensionProperties[i];
                buf.append(id.toStringInStack(stack, pos));
            }
            buf.append("]");
        }
        buf.append(" >");
