...
public abstract class ValidatorImpl implements Validator {
    ...
    public Exp validate(Exp exp, boolean scalar) {
        ...
        try {
            ...
            throw getPrintStackEx(Util.newInternal(
                e,
        ...
        if (scalar) {
            ...
            if (!TypeUtil.canEvaluate(type)) {
                ...
                throw getPrintStackEx(MondrianResource.instance().MdxMemberExpIsSet.ex(
                    exprString));
            }
        }
        ...
        Syntax syntax)
    {
        ...
        case 0:
            throw getPrintStackEx(MondrianResource.instance().NoFunctionMatchesSignature.ex(
                    signature));
        case 1:
            break;
        default:
            ...
            throw getPrintStackEx(MondrianResource.instance()
        ...
        String description)
    {
        ...
        if (definition) {
            ...
            throw getPrintStackEx(MondrianResource.instance().UnknownParameter.ex(name));
        }
    }
    ...
    public class PrintStackForTroubleshooting extends RuntimeException {
        public PrintStackForTroubleshooting(String message, Throwable cause) {
            super(message, cause);
        }
    }

    public RuntimeException getPrintStackEx(Throwable cause) {
        if (stack.size() > 0) {
            return new PrintStackForTroubleshooting(stackToString(), cause);
        } else {
    ...
    public String stackToString() {
        ...
        buf.append("stack.size: ").append(stack.size());
        if (stack.size() > 0) {
            buf.append(Util.nl).append("*-*-* Stack Top *-*-*");
            for (int i = stack.size() - 1; i >= 0; i--) {
                buf.append(Util.nl);
                QueryPart queryPart = stack.get(i);
                buf.append("[").append(i).append("]").append(" ").append(queryPart.toStringInStack(stack, i));
            }
            buf.append(Util.nl).append("*-*-* Stack Bottom *-*-*");
        }
