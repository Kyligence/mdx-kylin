...
public class Util extends XOMUtil {
     ...
     * @param queueMaxSize The maximum size of task queue, {@link ArrayBlockingQueue} in default, {@link SynchronousQueue} for 0.
    ...
    public static ExceptionCancellingAllTasksThreadPoolExecutor getExceptionCancellingExecutorService(
            int maximumPoolSize,
            int corePoolSize,
            long keepAliveTime,
            int queueMaxSize,
            final String name,
            RejectedExecutionHandler rejectionPolicy) {
        return (ExceptionCancellingAllTasksThreadPoolExecutor)getExecutorServiceInternal(
                maximumPoolSize,
                corePoolSize,
                keepAliveTime,
                queueMaxSize,
    ...
    private static ExecutorService getExecutorServiceInternal(
            int maximumPoolSize,
            int corePoolSize,
            long keepAliveTime,
            int queueMaxSize,
            final String name,
            RejectedExecutionHandler rejectionPolicy,
            boolean useExceptionCancellingPool)
    {
        if (Util.PreJdk16) {
        ...
        if (useExceptionCancellingPool) {
            executor =
                    new ExceptionCancellingAllTasksThreadPoolExecutor(
                            corePoolSize,
                            maximumPoolSize > 0
                                    ? maximumPoolSize
                                    : Integer.MAX_VALUE,
                            ...
                            // executor.
                            queueMaxSize <= 0 ? new SynchronousQueue<>() : new ArrayBlockingQueue<>(queueMaxSize),
                            factory);
        } else {
            executor =
    ...
    public static String makeFqNameWithAnd(OlapElement parent, String name) {
        if (parent == null) {
            return Util.quoteMdxIdentifier(name);
        } else {
            StringBuilder buf = new StringBuilder(64);
    ...
    public static String escapeFuzzySearch(String val) {
...
//        Arrays.sort(a, (Comparator<? super Object>) comparator);
    ...
    public static void checkCJResultLimit(long resultSize) {
        ...
        checkCJResultLimit(resultSize, resultLimit);

    }

    public static void checkCJResultLimit(long resultSize, long resultLimit) {
    ...
    public static interface Function3<PT1, PT2, PT3, RT> {
        RT apply(PT1 param1, PT2 param2, PT3 param3);
    }

    public static interface Function4<PT1, PT2, PT3, PT4, RT> {
        RT apply(PT1 param1, PT2 param2, PT3 param3, PT4 param4);
    }
    ...
    public static String getNlBySqlFormatted() {
        String separator = null;
        if (MondrianProperties.instance().GenerateFormattedSql.booleanValue()) {
            separator = nl;
        } else {
            separator = " ";
        }
    ...
    public static void getMeasureGroupList(List<RolapMeasureGroup> measureGroupList, Exp exp) {
        Set<Member> members = new LinkedHashSet<>();
        extractAllBaseMembers(exp, members);
        extractMeasureGroupsFromMeasures(measureGroupList, members);
    }

    public static void extractMeasureGroupsFromMeasures(List<RolapMeasureGroup> measureGroupList, Collection<Member> members) {
        for (Member member : members) {
            if (!(member instanceof RolapStoredMeasure)) {
                continue;
            }
            RolapMeasureGroup currentMeasureGroup = ((RolapStoredMeasure)member).getMeasureGroup();
            if (!measureGroupList.contains(currentMeasureGroup)) {
                measureGroupList.add(currentMeasureGroup);
            }
        }
    }
     ...
     */
    public static void extractAllBaseMembers(Exp exp, Collection<Member> members) {
        if (exp instanceof MemberExpr) {
            Member member = ((MemberExpr)exp).getMember();
            if (member instanceof RolapCalculatedMember) {
                extractAllBaseMembers(member.getExpression(), members);
            } else {
                members.add(member);
            }
            return;
        ...
        if (exp instanceof ResolvedFunCall) {
            for (Exp arg : ((ResolvedFunCall)exp).getArgs()) {
                extractAllBaseMembers(arg, members);
            }
        }
        if (exp instanceof NamedSetExpr) {
            extractAllBaseMembers(((NamedSetExpr)exp).getNamedSet().getExp(), members);
        }
    }
}
